...
namespace scene
{
	...
	public:
        ...
        virtual core::position2d<s32> getScreenCoordinatesFrom3DPosition(
            const core::vector3dd & pos, ICameraSceneNode* camera=0) = 0;
		...
		virtual ISceneNode* getSceneNodeFromRayBB(const core::line3d<f32>& ray,
			core::array<ESCENE_NODE_TYPE> TypeList, bool bNoDebugObjects=false, ISceneNode* root=0 ) =0;
		...
		virtual bool get3DPositionFromScreenPos(u32 srcX,u32 srcY,float& x,float& y,float& z) = 0;
		//! 根据屏幕坐标获取三维点坐标,如果获取失败返回false.gsl-2012/08/30
		virtual bool get3DPositionFromScreenPos(u32 srcX,u32 srcY,core::vector3df& position) = 0;
		virtual bool GetWorldCoord(int ix, int iy, double fz, core::vector3df& coord) = 0;
		virtual void projectPointPixels(float x,float y,float z, float &proj_x_px, float &proj_y_px, float &proj_z_depth) =0;
	};
