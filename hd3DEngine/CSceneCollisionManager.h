...
namespace scene
{
	...
	public:
		...
		// 返回指定类型的SceneNode
		virtual ISceneNode* getSceneNodeFromRayBB(const core::line3d<f32>& ray,
			core::array<ESCENE_NODE_TYPE> TypeList, bool bNoDebugObjects=false, ISceneNode* root=0 );

		virtual void getSceneNodesFromRayBB(const core::line3d<f32>& ray,
        ...
        //! Calculates 2d screen position from a 3d position.
        virtual core::position2d<s32> getScreenCoordinatesFrom3DPosition(
            const core::vector3dd & pos, ICameraSceneNode* camera=0);
		...
		//! 根据屏幕坐标获取三维点坐标,如果获取失败返回false.gsl-2012/08/30
		virtual bool get3DPositionFromScreenPos(u32 srcX,u32 srcY,float& x,float& y,float& z);
		//! 根据屏幕坐标获取三维点坐标,如果获取失败返回false.gsl-2012/08/30
		virtual bool get3DPositionFromScreenPos(u32 srcX,u32 srcY,core::vector3df& position);
		virtual bool GetWorldCoord(int ix, int iy, double fz, core::vector3df& coord);

		// 根据XYZ，得到投影后坐标
		virtual void projectPointPixels(float x,float y,float z, float &proj_x_px, float &proj_y_px, float &proj_z_depth);
	...
	private:
		...
		void getPickedNodeBB(ISceneNode* root, core::line3df& ray, core::array<ESCENE_NODE_TYPE> typelist,
			bool bNoDebugObjects,
			f32& outbestdistance, ISceneNode*& outbestnode);

		void getPickedNodesBB(ISceneNode* root, core::line3df& ray, s32 bits,
			bool bNoDebugObjects, core::array<ISceneNode*>& outNodes);
