...
namespace {
QImage decodeWithReader(QImageReader& reader, const QSize& target)
{
...
QImage decodeScaledFile(const QString& path, const QSize& target)
{
    ...
    return decodeWithReader(reader, target);
}
...
QSize jpegSizeFromBytes(const QByteArray& bytes)
{
    QBuffer buffer;
    buffer.setData(bytes);
    if (!buffer.open(QIODevice::ReadOnly)) return QSize();
    QImageReader reader(&buffer, "JPG");
    return reader.size(); // 只解析 JPEG 头，不生成完整像素图。
}
}
    ...
    const QStringList& paths, const QSize& knownFrameSize, QStringList* skippedPaths)
{
    ...
    for (int i = 0; i < paths.size(); ++i) {
        ...
        // The application has already enumerated these paths while building the
        // mile model. When every frame has the same known size, another
        // exists()/isFile() pass would synchronously stat tens of thousands of
        // files on the mechanical disk before the first frame can be shown.
        if (!knownFrameSize.isValid() && (!info.exists() || !info.isFile())) {
        ...
        frame.filePath = knownFrameSize.isValid()
            ? QDir::cleanPath(paths.at(i))
...
QSharedPointer<PackSequenceFrameSource> PackSequenceFrameSource::create(
    const QString& packPath, bool verifyOnOpen, QString* errorMessage)
{
    try {
        QSharedPointer<PackSequenceFrameSource> result(new PackSequenceFrameSource());
        result->m_indexPath = ImagePackReader::resolveIndexPath(packPath);
        if (result->m_indexPath.isEmpty()) {
            if (errorMessage) {
                *errorMessage = QString::fromUtf8(
                    "\xE6\x9C\xAA\xE6\x89\xBE\xE5\x88\xB0\xE5\xBD\x93\xE5\x89\x8D ImagePack"
                    "\xEF\xBC\x9A\xE7\x9B\xAE\xE5\xBD\x95\xE4\xB8\xAD\xE5\xBF\x85\xE9\xA1\xBB"
                    "\xE4\xB8\x94\xE5\x8F\xAA\xE8\x83\xBD\xE6\x9C\x89\xE4\xB8\x80\xE4\xB8\xAA "
                    "img_<tag>.jph\xEF\xBC\x8C\xE6\x88\x96\xE7\x9B\xB4\xE6\x8E\xA5\xE4\xBC\xA0\xE5\x85\xA5"
                    "\xE8\xAF\xA5 .jph \xE6\x96\x87\xE4\xBB\xB6");
            }
            return QSharedPointer<PackSequenceFrameSource>();
        }

        result->m_verifyOnOpen = verifyOnOpen;
        ImagePackReader reader(result->m_indexPath, verifyOnOpen);
        if (reader.count() == 0) {
            if (errorMessage) {
                *errorMessage = QString::fromUtf8(
                    "ImagePack \xE4\xB8\xAD\xE6\xB2\xA1\xE6\x9C\x89\xE5\x9B\xBE\xE7\x89\x87");
            }
            return QSharedPointer<PackSequenceFrameSource>();
        }

        // 当前 Pack 索引不保存图片宽高，因此只读取第一帧 JPEG 头获取统一尺寸。
        // 不在启动阶段完整解码第一张图；真正缩略/高清解码仍由视口按需触发。
        const QSize frameSize = jpegSizeFromBytes(reader.readJpeg(0));
        if (!frameSize.isValid()) {
            if (errorMessage) {
                *errorMessage = QString::fromUtf8(
                    "ImagePack \xE7\xAC\xAC\xE4\xB8\x80\xE5\xB8\xA7 JPEG "
                    "\xE5\xA4\xB4\xE6\x97\xA0\xE6\xB3\x95\xE8\xA7\xA3\xE6\x9E\x90");
            }
            return QSharedPointer<PackSequenceFrameSource>();
        }

        // 元数据按 4096 条分页从当前 ImagePack API 读取；只保留轻量信息，不预读 JPEG 正文。
        const QVector<PACK_FRAME_INFO> infos = reader.frameInfos();
        result->m_frames.reserve(infos.size());
        for (const PACK_FRAME_INFO& info : infos) {
            SequenceFrameDescriptor frame;
            // 当前 Pack 格式只保存 sourceIndex/timeValue，不保存原始文件名。
            frame.imageName = QString::number(static_cast<qulonglong>(info.sourceIndex));
            frame.sourceKey = result->m_indexPath + QLatin1Char('#')
                + QString::number(static_cast<qulonglong>(info.globalIndex));
...
ImagePackReader* PackSequenceFrameSource::readerForCurrentThread()
{
    if (!m_threadReaders.hasLocalData()) m_threadReaders.setLocalData(new ImagePackReader(m_indexPath, m_verifyOnOpen));
...
void SequenceChunkItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*)
{
    ...
    const bool highResolution = m_view->sequenceShouldUseHighResolution(lod);
