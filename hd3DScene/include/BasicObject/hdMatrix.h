...
#include <stdio.h>
	...
	double BASICOBJECT_API average(double a[], int n);

	void BASICOBJECT_API identity_matrix(double m[16]);

	void BASICOBJECT_API MatAdd(const double *m1,const double *m2,double *result,int rol,int col);

	void BASICOBJECT_API MatMinus(const double *m1,const double *m2,double *result,int rol,int col);

	void BASICOBJECT_API mult(const double *m1, const double *m2, double *result, int i_1, int j_12, int j_2);

	void BASICOBJECT_API mult(const float *m1, const float *m2, float *result, int i_1, int j_12, int j_2);

	int BASICOBJECT_API invers_matrix(double *m1, int n);

	int BASICOBJECT_API invers_matrix(float *m1, int n);

	int BASICOBJECT_API MatInv(double *m1,int n);

	void BASICOBJECT_API transpose(double *m1, double *m2, int m, int n);

	void BASICOBJECT_API transpose(float *m1, float *m2,int m,int n);

	void BASICOBJECT_API MatTrs(double *srcMatrix,int rol,int col,double *dstMatrix);

	void BASICOBJECT_API MatMult(const double *m1,const double *m2, double *result,int i_1,int j_12,int j_2);

	void BASICOBJECT_API MatOut(char *fileName,double *mat,int rol,int col);
	//! 根据旋转角度,得到旋转矩阵R.角度单位是弧度
	void BASICOBJECT_API GetRotateMatByAngle(double phi,double omega,double kappa,double *R);
	//! 根据4*4矩阵获取旋转角度
	//void BASICOBJECT_API getRotationDegrees(double* mat4,double& rotX,double& rotY,double& rotZ);

	// 计算特征值函数
	BASICOBJECT_API int matrixEejcb(
	...
	class BASICOBJECT_API hdMatrix
	{
	public:
		hdMatrix(void);
