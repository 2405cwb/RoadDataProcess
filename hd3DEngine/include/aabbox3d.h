...
class aabbox3d
{
	public:
		...
		// 判断点在XOY平面上与box相交--add by zhubo 2014.08.05
		bool isPointXoYInside(const vector3d<T>& p)const
		{
			return (p.X >= MinEdge.X && p.X <= MaxEdge.X &&
				p.Y >= MinEdge.Y && p.Y <= MaxEdge.Y);
		}
		...
		aabbox3d<T> getIntersectsBox(const aabbox3d<T>& other) const
		{
			aabbox3d<T> intersectBox;
			if (intersectsWithBox(other))
			{
				intersectBox.MinEdge.X = max_(MinEdge.X,other.MinEdge.X);
				intersectBox.MinEdge.Y = max_(MinEdge.Y,other.MinEdge.Y);
				intersectBox.MinEdge.Z = max_(MinEdge.Z,other.MinEdge.Z);

				intersectBox.MaxEdge.X = min_(MaxEdge.X,other.MaxEdge.X);
				intersectBox.MaxEdge.Y = min_(MaxEdge.Y,other.MaxEdge.Y);
				intersectBox.MaxEdge.Z = min_(MaxEdge.Z,other.MaxEdge.Z);
			}
			return intersectBox;
		}
		...
		bool getIntersectionWithLine(const vector3d<T>& linePoint,
			...
			vector3d<T>& outStart,
			vector3d<T>& outEnd) const
		{
			int count = 0;	//交点个数
			vector3d<T> pt1,pt2,pt3;
			vector3d<T> normal;
			// 底面
			pt1.set(MinEdge.X,MinEdge.Y,MinEdge.Z);
			pt2.set(MaxEdge.X,MinEdge.Y,MinEdge.Z);
			pt3.set(MaxEdge.X,MaxEdge.Y,MinEdge.Z);
			normal = (pt2 - pt1).crossProduct(pt3 - pt1);
			normal.normalize();
			T t2 = normal.dotProduct(lineVect);
			T D = -pt1.dotProduct(normal);
			if (!equals(t2,(T)0.0))
			{
				T t =- (normal.dotProduct(linePoint) + D) / t2;
				outStart = linePoint + (lineVect * t);
				if (isPointInside(outStart))
				{
					count++;
				}
			}
			// 顶面
			pt1.set(MinEdge.X,MinEdge.Y,MaxEdge.Z);
			pt2.set(MaxEdge.X,MinEdge.Y,MaxEdge.Z);
			pt3.set(MaxEdge.X,MaxEdge.Y,MaxEdge.Z);
			normal = (pt2 - pt1).crossProduct(pt3 - pt1);
			normal.normalize();
			t2 = normal.dotProduct(lineVect);
			D = -pt1.dotProduct(normal);
			if (!equals(t2,(T)0.0))
			{
				T t =- (normal.dotProduct(linePoint) + D) / t2;
				if(count == 0)
				...
				{
					outEnd = linePoint + (lineVect * t);
					if (isPointInside(outEnd))
					{
						count++;
					}
				}
			...
			// 左面
			pt1.set(MinEdge.X,MinEdge.Y,MinEdge.Z);
			pt2.set(MinEdge.X,MaxEdge.Y,MinEdge.Z);
			pt3.set(MinEdge.X,MaxEdge.Y,MaxEdge.Z);
			normal = (pt2 - pt1).crossProduct(pt3 - pt1);
			normal.normalize();
			t2 = normal.dotProduct(lineVect);
			D = -pt1.dotProduct(normal);
			...
			// 右面
			pt1.set(MaxEdge.X,MinEdge.Y,MinEdge.Z);
			pt2.set(MaxEdge.X,MaxEdge.Y,MinEdge.Z);
			pt3.set(MaxEdge.X,MaxEdge.Y,MaxEdge.Z);
			normal = (pt2 - pt1).crossProduct(pt3 - pt1);
			normal.normalize();
			t2 = normal.dotProduct(lineVect);
			...
			pt1.set(MinEdge.X,MinEdge.Y,MinEdge.Z);
			pt2.set(MaxEdge.X,MinEdge.Y,MinEdge.Z);
			pt3.set(MaxEdge.X,MinEdge.Y,MaxEdge.Z);
			normal = (pt2 - pt1).crossProduct(pt3 - pt1);
			normal.normalize();
			t2 = normal.dotProduct(lineVect);
			D = -pt1.dotProduct(normal);
			...
			// 后面
			pt1.set(MinEdge.X,MaxEdge.Y,MinEdge.Z);
			pt2.set(MaxEdge.X,MaxEdge.Y,MinEdge.Z);
			pt3.set(MaxEdge.X,MaxEdge.Y,MaxEdge.Z);
			normal = (pt2 - pt1).crossProduct(pt3 - pt1);
			normal.normalize();
