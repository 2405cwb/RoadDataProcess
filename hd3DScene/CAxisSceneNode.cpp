...
namespace hd
{
	namespace scene
	{
		video::SColor g_selColor = video::SColor(255,0,0,255);//黄色(255,255,255,0) 蓝色(255,0,0,255)

		CAxisSceneNode::CAxisSceneNode(ISceneNode* parent,ISceneManager* mgr,s32 id )
		...
		CAxisSceneNode::~CAxisSceneNode(void)
		{

		}

		void CAxisSceneNode::OnRegisterSceneNode()
		{
			if (IsVisible)
			{
		...
		void CAxisSceneNode::render()
		{
			...
			p3Dview->GetAxisPoint(centerPt.X,centerPt.Y);
			...
			// 2013/8/17 蔡红云重新根据三维坐标点来画X，Y，Z字体，以前的方法
			//driver->draw2DLine(centerPt, xSCPos, m_Xcolor);
			//driver->draw2DLine(centerPt, ySCPos, m_Ycolor);
			//driver->draw2DLine(centerPt, zSCPos, m_Zcolor);

			//gui::IGUIFont* pGUIFont = getSceneManager()->GetBuiltInFont();
			//core::dimension2d<s32> dim(10,10);
			//core::recti rtX(xSCPos, dim);
			//pGUIFont->draw("X", rtX, m_Xcolor, true, true);
			//core::recti rtY(ySCPos, dim);
			//pGUIFont->draw("Y", rtY, m_Ycolor, true, true);
			//core::recti rtZ(zSCPos, dim);
			///*pGUIFont->draw("Z", rtZ, video::SColor(255,0,0,255), true, true);*/
			...
			int		iSlices = 10;
			int		iStacks = 2;
			driver->DisplayAxis(lastNearIntersection, dAxisLength, dAxisRadius, dArrowLength,
			...
			//在target处画点
			core::vector3df camTarget = getSceneManager()->getActiveCamera()->getTarget();
			core::position2di screenPos = SceneManager->getSceneCollisionManager()->getScreenCoordinatesFrom3DPosition(camTarget);
			f32 nPtSize = 5.0f;//画点时，点的大小，又像素为单位计算
			driver->draw2DLine(core::vector2di(screenPos.X - (s32)nPtSize, screenPos.Y),
				core::vector2di(screenPos.X + (s32)nPtSize, screenPos.Y),video::SColor(255,255,0,0));
			driver->draw2DLine(core::vector2di(screenPos.X, screenPos.Y - (s32)nPtSize),
				core::vector2di(screenPos.X, screenPos.Y + (s32)nPtSize),video::SColor(255,255,0,0));
			driver->draw2DPolygon(screenPos, nPtSize,video::SColor(255,255,0,0),16);
			driver->drawPixel(screenPos.X, screenPos.Y, video::SColor(255,255,0,0));
		}

		const core::aabbox3d<f32>& CAxisSceneNode::getBoundingBox() const
		{
			return m_box;
		}

		irr::u32 CAxisSceneNode::getMaterialCount() const
		{
			return 1;
		}

		video::SMaterial& CAxisSceneNode::getMaterial( u32 i )
		{
			return m_material;
		}
