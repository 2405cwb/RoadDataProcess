	...
	FileSystem(fs), UserReceiver(0), Operator(op)
{
	...
	// 加载英文字体
	loadBuiltDefaultFont();

	// 加载中文字体
	loadBuiltChineseFont();
...
void CGUIEnvironment::loadBuiltDefaultFont()
{
	//io::path fontPath = "#DefaultFont";
	//io::IReadFile* file = io::createMemoryReadFile(BuiltInFontData, BuiltInFontDataSize, fontPath, false);

	char fontPath[512] = {0};
	GetWindowsDirectory(fontPath,512);
	strcat(fontPath,"\\fonts\\Arial.ttf");		// 

	EHD_FONT_SIZE nFontSize = SIZE_FONT_NORMAL_4;	// 初始创建时的字体大小
	gui::CGUITTFont* font  = CGUITTFont::createTTFont(this, fontPath, nFontSize);
	if (font)
	{
		SFont f;
		f.NamedPath.setPath(fontPath);
		f.Font = font;
		Fonts.push_back(f);
	}
	//file->drop();
...
void CGUIEnvironment::loadBuiltChineseFont()
{
	char fontPath[512] = {0};
	GetWindowsDirectory(fontPath,512);
	strcat(fontPath,"\\fonts\\simsun.ttc");		// 宋体simsun.ttc
...
IGUIFont* CGUIEnvironment::getBuiltDefaultFont() const
{
	if (Fonts.empty())
		return 0;
...
// Environment初始时增加中文字体 也可以在后面用上面的addFont函数添加
IGUIFont* CGUIEnvironment::getBuiltChineseFont() const
{
	if (Fonts.size() <= 1)
	{
		return NULL;
