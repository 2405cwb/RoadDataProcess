	...
	public:
		...
		*/
				...
				const plane3d<T>& o2, vector3d<T>& outPoint) const
		{
			...
			// 求点投影到平面后的结果
		bool projectPoint(const vector3d<T>& SourcePoint , vector3d<T>& OutPoint )
		{

			// 构造一条直线
	       return		getIntersectionWithLine(SourcePoint,Normal,	 OutPoint	);
		}
...
//! Typedef for an f64 3d plane.
typedef plane3d<f64> plane3dd;
