...
#include "hnDmiOffsetCorrection.h"
...
namespace hnPro
{
	static bool dmiValuesFitProjectRange(const vector<hnMilePile>& piles, const vector<hnMarkInfo>& marks, double offset, double length, bool requireOutside)
	{
		const double tolerance = qMax(2.0, length * 0.001); bool hasValue = false; bool hasOutside = false;
		for (const auto& pile : piles) { hasValue = true; hasOutside = hasOutside || pile.dEnclMile > length + tolerance; const double value = pile.dEnclMile - offset; if (value < -tolerance || value > length + tolerance) return false; }
		for (const auto& mark : marks) { hasValue = true; hasOutside = hasOutside || mark.dEnclMile > length + tolerance; const double value = mark.dEnclMile - offset; if (value < -tolerance || value > length + tolerance) return false; }
		return hasValue && (!requireOutside || hasOutside);
	}

	static double legacyDmiOffsetCandidate(const hnCommon::hnProjectSetInfo& setting, hn2DProject* project2d)
	{
		const double length = setting.dEndEnclMile  > 0.0 ? setting.dEndEnclMile:setting.dLength  ;
		if (project2d) { const hnDmiOffsetCorrection correction = hnDmiOffsetCorrection::analyzeMileStoneFile(project2d->getMileStoneCaliInfoFilePath(), length); if (correction.shouldCorrect) return correction.offset; }
		return setting.dBegEnclMile;
	}
	static bool hasValidMilePileSource(const QString& filePath)
	{
		QFile file(filePath);
		...
		while (!in.atEnd())
		{
			const QStringList fields = in.readLine().simplified().split(QStringLiteral(" "), QString::SkipEmptyParts);
			bool dmiOk = false;
			bool mileOk = false;
			if (fields.size() >= 2)
			{
				fields.at(0).toDouble(&dmiOk);
				fields.at(1).toDouble(&mileOk);
			}
			if (dmiOk && mileOk)
			{
				return true;
	...
	static bool hasValidMarkSource(const QString& filePath)
	{
		...
		while (!in.atEnd())
		{
			...
			if (fields.size() >= 4)
			{
				fields.at(2).toDouble(&dmiOk);
			}
			if (dmiOk && (fields.at(3).contains(QStringLiteral(":")) || fields.at(3).contains(QStringLiteral("："))))
			{
				return true;
			}
		}
	...
	static bool writeFieldSourceImportedFlag(const QString& flagPath)
	{
		QFile importFlag(flagPath);
...
#pragma endregion 
			...
			if (getResultDB(dbDirPath, dbFilePath, false))
			{
				...
				if (!milePileLoadedFromDb)
				{
					...
					milePileLoadedFromDb = true;
				}
				// The result database is the runtime source of truth.  The legacy flag only
				// records an old import and must never decide the stored DMI format.
				const bool databaseHasRuntimeData = !m_vecMarkInfo.empty() || !m_vecMileagePile.empty();
				if (databaseHasRuntimeData)
				{
					QString dmiStateError;
					const int dmiVersion = m_pDbSqlite->dmiStorageVersion(&dmiStateError);
					if (dmiVersion == -2)
					{
						QMessageBox::critical(QApplication::activeWindow(), QStringLiteral("DMI 数据待修复"),
							QStringLiteral("无法读取成果库 DMI 存储状态：%1").arg(dmiStateError));
						return false;
					}
					if (dmiVersion != 1)
					{
						const double dmiLength = m_projectInfo.dEndEnclMile > 0.0 ? m_projectInfo.dEndEnclMile : m_projectInfo.dLength;
						const double candidateOffset = legacyDmiOffsetCandidate(m_projectInfo, m_p2DProject);
						const bool relativeDmi = dmiValuesFitProjectRange(m_vecMileagePile, m_vecMarkInfo, 0.0, dmiLength, false);
						const bool absoluteDmi = candidateOffset > 0.0 && dmiValuesFitProjectRange(m_vecMileagePile, m_vecMarkInfo, candidateOffset, dmiLength, dmiVersion != 0);
						if (absoluteDmi && (dmiVersion == 0 || !relativeDmi))
						{
							QString migrateError;
							if (!repairLeadingDmiOffset(candidateOffset, &migrateError))
							{
								QMessageBox::critical(QApplication::activeWindow(), QStringLiteral("DMI 数据待修复"),
									QStringLiteral("旧绝对 DMI 迁移失败，工程将保持只读：%1").arg(migrateError));
								return false;
							}
							m_vecMarkInfo.clear(); m_vecMileagePile.clear();
							if (!m_pDbSqlite->m_markerInfoTable.readData(m_vecMarkInfo) || !m_pDbSqlite->m_milePileTable.readData(m_vecMileagePile)) return false;
						}
						else if (relativeDmi)
						{
							if (!m_pDbSqlite->setDmiStorageVersion(1, 0.0, QStringLiteral("verified relative DMI legacy database"), &dmiStateError))
							{
								QMessageBox::critical(QApplication::activeWindow(), QStringLiteral("DMI 数据待修复"), dmiStateError);
								return false;
							}
						}
						else
						{
							QMessageBox::critical(QApplication::activeWindow(), QStringLiteral("DMI 数据待修复"),
								QStringLiteral("无法唯一判断成果库 DMI 是绝对值还是相对值；未修改任何数据。请使用备份和诊断工具修复后再打开工程。"));
							return false;
						}
					}
				}

				// Text files are only a one-time seed for an empty result database.
				const QString fieldMarkPath = m_str2DProPath + QStringLiteral("/RoadStatuMarkInfo.txt");
				const QString fieldMilePilePath = m_str2DProPath + QStringLiteral("/Dmi2Mile.txt");
				if (m_vecMarkInfo.empty() && hasValidMarkSource(fieldMarkPath))
					m_bNeedImportFieldMarksToResultDb = true;
				if (m_vecMileagePile.empty() && hasValidMilePileSource(fieldMilePilePath))
	...
	{
		if (!m_pDbSqlite || !m_pDbSqlite->executeDB("BEGIN IMMEDIATE;")) return false;
		if (!m_pDbSqlite->m_markerInfoTable.clearData() || !m_pDbSqlite->m_markerInfoTable.writeData(m_vecMarkInfo) || !m_pDbSqlite->executeDB("COMMIT;"))
		{
			m_pDbSqlite->executeDB("ROLLBACK;"); return false;
		}
		return true;
	}
	...
	{
		if (!m_pDbSqlite || !m_pDbSqlite->executeDB("BEGIN IMMEDIATE;")) return false;
		if (!m_pDbSqlite->m_milePileTable.clearData() || !m_pDbSqlite->m_milePileTable.writeData(m_vecMileagePile) || !m_pDbSqlite->executeDB("COMMIT;"))
		{
			m_pDbSqlite->executeDB("ROLLBACK;"); return false;
		}
		return true;
	...
	{
		if ((!m_bNeedImportFieldMilePilesToResultDb && !m_bNeedImportFieldMarksToResultDb) || !m_p2DProject || !m_pDbSqlite) return;
		if (!m_pDbSqlite->executeDB("BEGIN IMMEDIATE;"))
		{
			QMessageBox::warning(QApplication::activeWindow(), QStringLiteral("外业数据导入"), QStringLiteral("无法启动成果库事务。"));
			return;
		}
		...
		{
			const QString milePilePath = m_p2DProject->getMilePilePath();
			if (!hasValidMilePileSource(milePilePath)) { importSuccess = false; importErrors << QStringLiteral("Dmi2Mile.txt 不存在或没有有效较桩记录。"); }
			else
			{
				m_p2DProject->add2dMilePile(m_vecMileagePile);
				sort(m_vecMileagePile.begin(), m_vecMileagePile.end(), compareMilepileByEnclMile);
				if (!m_pDbSqlite->m_milePileTable.clearData() || !m_pDbSqlite->m_milePileTable.writeData(m_vecMileagePile)) { importSuccess = false; importErrors << QStringLiteral("较桩数据写入成果库失败。"); }
			}
		}
		if (importSuccess && m_bNeedImportFieldMarksToResultDb)
		{
			const QString markFilePath = m_p2DProject->getMarkFilePath();
			if (!hasValidMarkSource(markFilePath)) { importSuccess = false; importErrors << QStringLiteral("RoadStatuMarkInfo.txt 不存在或没有有效打标记录。"); }
			else
			{
				m_p2DProject->add2dMarkInfo(m_vecMarkInfo, this);
				if (!m_pDbSqlite->m_markerInfoTable.clearData() || !m_pDbSqlite->m_markerInfoTable.writeData(m_vecMarkInfo)) { importSuccess = false; importErrors << QStringLiteral("打标数据写入成果库失败。"); }
			}
		}
		QString stateError;
		if (importSuccess && !m_pDbSqlite->setDmiStorageVersion(1, 0.0, QStringLiteral("initial field source import normalized to relative DMI"), &stateError))
		{
			importSuccess = false; importErrors << stateError;
		}
		if (!importSuccess || !m_pDbSqlite->executeDB("COMMIT;"))
		{
			m_pDbSqlite->executeDB("ROLLBACK;");
			QMessageBox::warning(QApplication::activeWindow(), QStringLiteral("外业数据导入失败"),
				importErrors.join(QStringLiteral("\n")) + QStringLiteral("\n本次操作已经回滚，成果库没有被部分更新。"));
			return;
		}
		// Kept only for backwards compatibility; runtime code never reads this flag.
		writeFieldSourceImportedFlag(m_strDbDirPath + "/FieldSourceImported.flag");
		m_bNeedImportFieldMilePilesToResultDb = false;
		m_bNeedImportFieldMarksToResultDb = false;
	}
 ...
 void hnProject::setCurrentRoadMile(const hnMile & mile)
	...
	void hnProject::changeMilePile(QVector < hnCommon::hnMilePile>& piles)
	{
		...
		sort(m_vecMileagePile.begin(), m_vecMileagePile.end(), compareMilepileByEnclMile);
		if (!saveMileagePilesToResultDb()) { m_vecMileagePile = previousPiles; return; }
		initMileList(false);
	}
	...
	bool hnProject::repairLeadingDmiOffset(double offset, QString* errorMessage)
	{
		if (!m_pDbSqlite || offset <= 0.0) return false;
		const QString backupPath = m_strDbFilePath + QStringLiteral(".dmi-offset-backup-") + QDateTime::currentDateTime().toString(QStringLiteral("yyyyMMddhhmmss"));
		if (!QFile::copy(m_strDbFilePath, backupPath)) { if (errorMessage) *errorMessage = QStringLiteral("database backup failed"); return false; }
		QFile::setPermissions(backupPath, QFile::ReadOwner | QFile::ReadUser | QFile::ReadGroup | QFile::ReadOther);
		return m_pDbSqlite->repairLeadingDmiOffset(offset, errorMessage);
	}


	bool hnProject::updateProjectSettings(const hnCommon::hnProjectSetInfo& settings, QString* errorMessage)
	{
		if (!m_pDbSqlite || !m_pDbSqlite->isOpen()) { if (errorMessage) *errorMessage = QStringLiteral("结果数据库不可用."); return false; }
		if (settings.dBegMile == settings.dEndMile || (settings.nLineType == 1 && settings.dBegMile > settings.dEndMile) || (settings.nLineType != 1 && settings.dBegMile < settings.dEndMile)) { if (errorMessage) *errorMessage = QStringLiteral("Invalid start/end mileage for direction."); return false; }
		if (strlen(settings.strRoadStandard) == 0 || strlen(settings.strRoadLevel) == 0) { if (errorMessage) *errorMessage = QStringLiteral("需要道路标准和道路水平."); return false; }
		hnCommon::hnProjectSetInfo candidate = settings; candidate.dLength = qAbs(candidate.dEndMile - candidate.dBegMile);
		if (!m_pDbSqlite->executeDB("BEGIN IMMEDIATE;")) { if (errorMessage) *errorMessage = QStringLiteral("无法启动数据库事务."); return false; }
		if (!m_pDbSqlite->m_projectSetTable.writeData(candidate)) { m_pDbSqlite->executeDB("ROLLBACK;"); if (errorMessage) *errorMessage = QStringLiteral("无法编写项目设置 ."); return false; }
		if (!m_pDbSqlite->executeDB("COMMIT;")) { m_pDbSqlite->executeDB("ROLLBACK;"); if (errorMessage) *errorMessage = QStringLiteral("无法提交项目设置 ."); return false; }
		m_projectInfo = candidate; initMileList(false); return true;
	}
	...
	bool hnProject::ensureInitialSurfaceMaterial(QWidget* parent, QString* errorMessage)
	{
		...
		hnCommon::hnMarkInfo mark; mark.nType = 0; mark.dTrueMile = startMile; mark.dEnclMile = trueMileToEncl(startMile); mark.dGpsTimer = -2; strncpy_s(mark.strMark, material.toLocal8Bit().constData(), _TRUNCATE); addMark(mark); return true;
	}}
