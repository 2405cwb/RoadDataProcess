...
void Gauss_Btox(double a,double f,double h,double Bm, double B, double L,double L0,double &m_x,double &m_y)
{
	...
	//	e1=(a*a-b*b)/(a*a);
	e2=(a*a-b*b)/(a*a);
	//double e20=1-pow((1-(1.0/f)),2);

	//double B0=0;
	//B0=30.0*PI/180;
	//da =h * (1.0 - e2*pow(sin(B),2)) / sqrt(1.0 - e2);

	// 更换计算方法;
	da = h * (1.0) / sqrt(1.0 - e2);

	////double e1 = ;
	//double dW = sqrt(1.0 - e2 * pow(sin(B),2));
	//double dNw = a / dW;
	//double ne2 = e2 * dNw / (dNw + h);
	//double a2a = (dNw + h)/(sqrt(1 - ne2 * pow(sin(B),2)));
	//e2 = ne2;
	////double da2 = h /(sqrt(1.0 - e2*pow(sin(B),2)));
	...
	//m_x=X0-M0+0.5*N*t*pow(m0,2.0)+1.0/24.0*(5-pow(t,2)+9*n2+4*pow(n2,2.0))*N*t*pow(m0,4.0)
	//	+1.0*(61.0-58.0*pow(t,2.0)+pow(t,4.0))*N*t*pow(m0,6.0) /(720.0* pow(180.0*3600.0/PI,6.0) );
	...
	//// test check;
	//double xm0 = a*(1-e2);
	//double xm2 = 3*e2*xm0/2;
	//double xm4 = 5*e2*xm2/4;
	//double xm6 = 7*e2*xm4/6;
	//double xm8 = 9*e2*xm6/8;

	//double xa0 = xm0 + xm2*1.0/2 + 3*xm4 / 8 + 5 * xm6 / 16 + 35 * xm8 / 128;
	//double xa2 = xm2/2 + xm4/2 + 15 * xm6 / 32 + 7 * xm8 / 16;
	//double xa4 = xm4 / 8 + 3* xm6 / 16 + 7 * xm8 / 32;
	//double xa6 = xm6 / 32 + xm8 / 16;
	//double xa8 = xm8 / 128;

	////X0 = xa0*B - xa2 * sin(2*B) / 2 + xa4 * sin(4*B) / 4 - xa6 * sin(6*B) / 6 + xa8 * sin(8*B) / 8;
	//double X01 =  xa0*B - sin(B)*cos(B)*(xa2 - xa4 + xa6 + (2.0 * xa4 - 16.0 * xa6 / 3.0)* pow(sin(B),2.0) + 16.0 * (xa6 * pow(sin(B),4.0))/3.0);


	//m_x = X0 + N * sin(B)*cos(B) * pow(l,2.0)/(2.0 * pow(180.0*3600/PI,2.0)) + N * sin(B) * pow(cos(B),3.0) * (5-pow(t,2)+9*n2+4*pow(n2,2.0))*pow(l,4.0) /(24.0*pow(180.0*3600/PI,4.0))
	//	+ N * sin(B)*pow(cos(B),5.0)*(61.0-58.0*pow(t,2.0)+pow(t,4.0)) * pow(l,6.0) / (720.0*pow(180.0*3600/PI,6.0));
