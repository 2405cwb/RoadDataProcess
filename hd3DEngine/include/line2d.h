...
class line2d
{
	public:
      ...
      // two lines insect  not line segment
     bool intersectWithLine(const line2d<T>& l, vector2d<T>& out) const
     {
			// Uses the method given at:
			// http://local.wasp.uwa.edu.au/~pbourke/geometry/lineline2d/ 
			const f32 commonDenominator = (f32)((l.end.Y - l.start.Y)*(end.X - start.X) -
		...
		//! Get the closest point on this line to a point,可能为延长线上
		vector2d<T> getClosestPointT(const vector2d<T>& point,T& td) const
		{
			vector2d<T> c = point - start;
			vector2d<T> v = end - start;
			T d = (T)v.getLength();
