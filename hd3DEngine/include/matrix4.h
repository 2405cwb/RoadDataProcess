...
namespace core
{
	...
	class CMatrix4
	{
		public:
			...
			//! 设置姿态角度Yaw\Pitch\Roll
			CMatrix4<T>& SetHPR(double yaw,double pitch,double roll);

			//! 获取姿态角度Yaw\Pitch\Roll
			void GetHPR(double& yaw, double& pitch, double& roll);
			...
			//! Rotate a vector by the rotation part of this matrix.
			void rotateVect(f32& x,f32& y,f32& z) const;
			...
			*/
			...
			// 根据旋转矩阵计算iScan的yaw/pitch/roll(度为单位)
			void GetHPRFromRMatrix(double& heading, double& pitch, double& roll, double* R);
			// 根据iScan的yaw/pitch/roll(度为单位)计算旋转矩阵
			void GetRMatrixFromHPR(double heading, double pitch, double roll, double *R);

			// 根据四元数计算旋转矩阵 yangfeng 2014/04/23
			void GetRMatrixFromQuaternion(double Q0, double Q1, double Q2, double Q3, double *R);
	...
	*/
	...
	//! 设置姿态角度Yaw\Pitch\Roll
	template <class T>
	inline CMatrix4<T>& CMatrix4<T>::SetHPR(double yaw,double pitch,double roll)
	{
		double R[9] = {0.0};
		GetRMatrixFromHPR(yaw, pitch,roll, R);
	...
	template < class T >
	inline void CMatrix4<T>::GetHPR(double& yaw, double& pitch, double& roll)
	{
		double R[9] = {0.0};
		R[0] = M[0]; R[3] = M[1]; R[6] = M[2];
		R[1] = M[4]; R[4] = M[5]; R[7] = M[6];
		R[2] = M[8]; R[5] = M[9]; R[8] = M[10];
		GetHPRFromRMatrix(yaw, pitch, roll, R);
	}

	// 根据旋转矩阵计算iScan的yaw/pitch/roll(度为单位)
	template <class T>
	inline void CMatrix4<T>::GetHPRFromRMatrix(double& heading, double& pitch, double& roll, double* R)
	{
		pitch = asin(R[7]);
		roll = atan2(-R[6], R[8]);
		heading = atan2(R[1], R[4]);

		// 将弧度转为度
		heading = heading/PI64 * 180.0;
		pitch = pitch/PI64 * 180.0;
		roll = roll/PI64 * 180.0;
	}

	// 根据iScan的yaw/pitch/roll(度为单位)计算旋转矩阵
	template <class T>
	inline void CMatrix4<T>::GetRMatrixFromHPR(double heading, double pitch, double roll, double *R)
	{
		// 将度转位弧度
		heading = heading/180.0*PI64;
		pitch = pitch/180.0*PI64;
		roll = roll/180.0*PI64;

		//旋转矩阵
		double sinH = sin(heading);   double cosH = cos(heading);
		double sinP = sin(pitch);   double cosP = cos(pitch);
		double sinR = sin(roll);   double cosR = cos(roll);

		//旋转矩阵
		R[0] =  cosH*cosR + sinH*sinP*sinR;
		R[1] = sinH*cosP;		
		R[2] = cosH*sinR - sinH*cosR*sinP;

		R[3] =  cosH*sinP*sinR - sinH*cosR;							
		R[4] =  cosH*cosP;							
		R[5] = - sinH*sinR - cosH*cosR*sinP;
	...
	// 根据四元数计算旋转矩阵 yangfeng 2014/04/23
	// 单位四元数定义为超复数 q = d + ia + jb + kc
	// 其中 d*d + a*a + b*b + c*c = 1
	template <class T>
	inline void CMatrix4<T>::GetRMatrixFromQuaternion(double Q0, double Q1, double Q2, double Q3, double *R)
	{
		// 旋转矩阵
	...
	*/
	...
	template <class T>
	inline void CMatrix4<T>::rotateVect(f32& x,f32& y,f32& z) const
	{
		vector3df tmp(x,y,z);
		x = tmp.X*M[0] + tmp.Y*M[4] + tmp.Z*M[8];
		y = tmp.X*M[1] + tmp.Y*M[5] + tmp.Z*M[9];
		z = tmp.X*M[2] + tmp.Y*M[6] + tmp.Z*M[10];
	}
