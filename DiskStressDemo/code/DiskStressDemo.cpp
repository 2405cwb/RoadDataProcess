...
public:
	CDiskStressApp()
		...
		, m_packSerialNo(0)
		, m_packCurrentNo(-1)
		, m_packIndexCreateTimeValue(0)
		, m_packTotalFrameCount(0)
		, m_packDatFileCount(0)
...
private:
	...
	bool Prepare()
	{
		...
		{
			if (!OpenPackIndexFile())
			{
				std::cout << "failed to open PackIndex.idx." << std::endl;
				m_log.PrintLine("failed to open PackIndex.idx.");
				return false;
			}
		}
	...
	void FillCurrentPackIdxHeader(DEMO_IDX_FILE_HEADER& header)
	{
		...
		memcpy(header.magic, "IOVIDX2", 7);
		header.version = 2;
		header.headerSize = sizeof(header);
		header.recordSize = sizeof(DEMO_IDX_RECORD);
		header.datFileCount = (DWORD)m_packDatFileCount;
		header.recordCount = (unsigned long long)m_packTotalFrameCount;
		header.createTimeValue = m_packIndexCreateTimeValue;
	}

	bool OpenPackIndexFile()
	{
		...
		std::string idxPath = JoinPath(m_runOutputDir, "PackIndex.idx");
		FILE* idxFile = NULL;
		...
		m_packIndexCreateTimeValue = ParseFrameTimeValue(MakeFrameTimeString().c_str());
		m_packTotalFrameCount = 0;
		m_packDatFileCount = 0;
	...
	bool WritePackFrame(const JPEG_WRITE_TASK& task, PACK_WRITE_METRIC& metric)
	{
		...
		idxRecord.datNo = (DWORD)m_packCurrentNo;
	...
	bool RotatePackFilesIfNeeded(unsigned long jpegSize, PACK_WRITE_METRIC& metric)
	{
		...
		if (!CloseCurrentPackDataFileUnlocked(localCloseMs))
		{
			return false;
		}
		...
		if (datFile == NULL || m_packIndexFile == NULL)
		{
			if (datFile != NULL)
			{
				fclose(datFile);
	...
	bool CloseCurrentPackDataFileUnlocked(DWORD& closeMs)
	{
		if (m_packDataFile == NULL)
		{
			m_packCurrentFrameCount = 0;
	...
	bool CloseCurrentPackFilesUnlocked(DWORD& closeMs)
	{
		...
		if (!CloseCurrentPackDataFileUnlocked(closeMs))
		{
			return false;
		}
...
private:
	...
	int m_packSerialNo;
	int m_packCurrentNo;
	unsigned long long m_packIndexCreateTimeValue;
	long long m_packTotalFrameCount;
	int m_packDatFileCount;
