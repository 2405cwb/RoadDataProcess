...
namespace hd
{
	...
	template <class T>
	class HNCOORDTRANSLATE_API hdMatrix4
	{
		public:
	...
	inline CHdVector3d<T> hdMatrix4<T>::getRotationDegrees() const
	{
		...
		const CHdVector3d<double> invScale(reciprocal(scale.X),reciprocal(scale.Y),reciprocal(scale.Z));
	...
	*/
	...
	inline bool hdMatrix4<T>::getInverse(hdMatrix4<T>& out) const
	{
		...
		float d = (float)((m(0, 0) * m(1, 1) - m(0, 1) * m(1, 0)) * (m(2, 2) * m(3, 3) - m(2, 3) * m(3, 2)) -
	...
	typedef hdMatrix4<float> matrix4;

	//! Typedef for double matrix
	typedef hdMatrix4<double> matrix4d;
