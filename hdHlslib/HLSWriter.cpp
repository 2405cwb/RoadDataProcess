...
BOOL HLSWriter::write( const char* file_name, const HLSheader* header,BOOL bUpdateHeader, U32 io_buffer_size/*=65536*/ )
{
	if (file_name == 0)
	{
	...
	SYSTEMTIME tm;
	GetSystemTime(&tm);
	m_header.file_creation_day = GetDayOfYear(tm.wMonth,tm.wDay, tm.wYear);
	m_header.file_creation_year = tm.wYear;

	size_t size = strlen(file_name);
	size_t size0 = sizeof(m_file1path);
	strncpy_s(m_file1path,size0, file_name,size);
	strncat_s(m_file1path,size0,"1",1);
