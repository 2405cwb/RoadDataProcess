...
namespace scene
{
	...
	*/
	struct SViewFrustum
	{
		...
		//! 判断和立方体是否相交
		bool isCubeIn(core::aabbox3d<f32>& cube) const;

        //! 判断和立方体是否有边相交
        bool isCubeClipped(core::aabbox3d<f32>& cube) const;
	...
	*/
	...
	//! 判断和立方体是否相交(立方体是否全部在视锥内)
	inline bool SViewFrustum::isCubeIn(core::aabbox3d<f32>& cube) const
	{
		//// 该判断可能会出现cube的Box范围完全包含视锥box，此种情况下会出错--2014.05.22 add by zhubo
		//core::vector3df viewFrustCenter = boundingBox.getCenter();
		//core::vector3df cubeCenter = cube.getCenter();
		//core::vector3df distView;
		//core::vector3df distCube;
		//core::vector3df rotCube;

		//// 计算视锥box的长宽高半径值
		//distView.X = abs(boundingBox.MaxEdge.X - boundingBox.MinEdge.X);
		//distView.Y = abs(boundingBox.MaxEdge.Y - boundingBox.MinEdge.Y);
		//distView.Z = abs(boundingBox.MaxEdge.Z - boundingBox.MinEdge.Z);
		//distView.X /= 2.0;
		//distView.Y /= 2.0;
		//distView.Z /= 2.0;

		//// 计算cube box的长宽高半径值
		//distCube.X = abs(cube.MaxEdge.X - cube.MinEdge.X);
		//distCube.Y = abs(cube.MaxEdge.Y - cube.MinEdge.Y);
		//distCube.Z = abs(cube.MaxEdge.Z - cube.MinEdge.Z);
		//distCube.X /= 2.0;
		//distCube.Y /= 2.0;
		//distCube.Z /= 2.0;

		//// 获得两个box的长宽高阈值
		//distView.X += distCube.X;
		//distView.Y += distCube.Y;
		//distView.Z += distCube.Z;

		//// 两个box中线点的距离值
		//rotCube.X = abs(viewFrustCenter.X - cubeCenter.X);
		//rotCube.Y = abs(viewFrustCenter.Y - cubeCenter.Y);
		//rotCube.Z = abs(viewFrustCenter.Z - cubeCenter.Z);

		//// 中心点距离值超过长宽高阈值，表明需要两box不相交
		//if (rotCube.X > distView.X || rotCube.Y > distView.Y || rotCube.Z > distView.Z)
		//{
		//	return false;
		//}
		//return true;
		...
		for (i = 0; i < VF_PLANE_COUNT; i++)
		{
			const core::plane3df& p = planes[i];

			// 如果该box在其前面，需进一步判断该box是否包含视锥box
			if(cube.classifyPlaneRelation(p) == core::ISREL3D_FRONT)
			{
				return false;
			}
		}
    ...
    //! 判断和立方体是否有边相交
    inline bool SViewFrustum::isCubeClipped(core::aabbox3d<f32>& cube) const
    {
        int i;
        for (i = 0; i < VF_PLANE_COUNT; i++)
        {
            ...
            // 如果该box在其前面，需进一步判断该box是否包含视锥box
            if(cube.classifyPlaneRelation(p) == core::ISREL3D_CLIPPED)
            {
                return true;
                break;
            }
