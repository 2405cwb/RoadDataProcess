...
bool Document::writeAndFormat(int row, int col, const QVariant & value, int formatRow)
{

	if (Worksheet *sheet = currentWorksheet()) 
	{
		auto format = sheet->cellAt(formatRow, col)->format();
		return sheet->write(row, col, value, format);
	} 
	return false;
}
...
void QXlsx::Document::deleteLastColumn(int colmun)
{
	int maxRow = this->dimension().lastRow();
	int maxClo = this->dimension().lastColumn();
	QXlsx::Format emptyFormat;
	//	emptyFormat.setBorderStyle(QXlsx::Format::BorderNone);
	emptyFormat.setTopBorderStyle(QXlsx::Format::BorderNone);
	emptyFormat.setBottomBorderStyle(QXlsx::Format::BorderNone);
	emptyFormat.setLeftBorderStyle(QXlsx::Format::BorderNone);
	emptyFormat.setRightBorderStyle(QXlsx::Format::BorderNone);
	//从要删除的列开始，将右侧列向左移动
	for (int col = colmun; col < maxClo; ++col)
	{
		for (int row = 1; row <= maxRow; ++row)
		{
			//获取右侧单元格的值
			QXlsx::Cell*cell = this->cellAt(row, col + 1);
			if (cell)
			{
				//复制
				this->write(row, col, cell->value(), emptyFormat);
			}
			else
			{
				this->write(row, col, QVariant(), emptyFormat);
			}
	...
	for (int row = 1; row <= maxRow; ++row)
	{
		this->write(row, maxClo, QVariant(), emptyFormat);

	}

}

void QXlsx::Document::deleteColumn(int colmun)
{
	int maxRow = this->dimension().lastRow();
	int maxClo = this->dimension().lastColumn();
	QXlsx::Format emptyFormat;
...
void QXlsx::Document::copyRange( const QString & sourceRange, const QString targetStart)
{
	 //this.selectSheet(sheetName);

	// 解析源范围和目标起点
	QRegularExpression re("([A-Z]+)(\\d+):([A-Z]+)(\\d+)");
	QRegularExpressionMatch match = re.match(sourceRange);
	if (!match.hasMatch()) return; // 如果源范围格式不正确，直接返回

	char startCol = match.captured(1).at(0).toLatin1();
	int startRow = match.captured(2).toInt();
	char endCol = match.captured(3).at(0).toLatin1();
	int endRow = match.captured(4).toInt();

	int targetStartRow = targetStart.mid(1).toInt();
	char targetStartCol = targetStart.at(0).toLatin1();

 

	// 循环遍历源范围内的单元格并复制到目标位置
	for (int row = startRow; row <= endRow; ++row) {
		for (char col = startCol; col <= endCol; ++col) {
			// 计算目标单元格位置
			int targetRow = targetStartRow + (row - startRow);
			char targetCol = targetStartCol + (col - startCol);
			
			QString originalCell = QString("%1%2").arg(col).arg(row);
			QString targetCell = QString("%1%2").arg(targetCol).arg(targetRow);

			// 读取原始单元格内容和格式
			QVariant content =this->read(originalCell); 
		Cell* cell = this->cellAt(originalCell);
			Format format;
			if (cell)
			{
			 format = cell->format();
			...
			// 将内容和格式复制到目标单元格
			this->write(targetCell, content, format);
		}
	}
	//xlsx.save();
}
...
QVariant QXlsx::Document::readCellValue(int row, int col) const
{
	if (Worksheet *sheet = currentWorksheet())
		return sheet->readCellValue(row, col);
	return QVariant();
}
...
void QXlsx::Document::reverseRowsFrom(int sCol, int eCol, int startRow, int endRow /*= 0*/)
{
	//int maxRow = this->dimension().lastRow();
	//if (endRow!=0)
	//{
	...
	//int rowCount = maxRow - startRow+1;
	//struct TempCellData
	//{
	//	QVariant value;
	//	QXlsx::Format format; 
	...
	////临时储存需要反转的行数据
	//QVector<QVector<TempCellData>> tempDatas(rowCount);
	//for (int i = 0 ; i<rowCount ;++i)
	//{
	//	int curRow = startRow + i;
	//	QVector<TempCellData> rowData;
	//	for (int col = sCol; col<= eCol;++col)
	//	{
	//		TempCellData cell;
	//		 auto curCell = this->cellAt(curRow, col);
	//	 
	//		cell.value = this->readCellValue(curRow, col);
	//		if (QXlsx::Cell* xlsxCell = this->cellAt(curRow,col))
	//		{
	//			cell.format = xlsxCell->format();
	//		}
	//		rowData.append(cell);
	//	}
	//	tempDatas[i]=rowData;
	...
	//	int sourceIndex = rowCount - 1 - i;
	//	for (int col = sCol; col <=eCol;++col)
	//	{ 
	//		const TempCellData & cell = tempDatas[sourceIndex][col - sCol];
	//		this->write(targetRow,  col,
	//			cell.value,
	//			cell.format);
	//	}
	//}
...
void QXlsx::Document::swapColumns(int startRow, int col1, int col2)
{
	int maxRow = this->dimension().lastRow();
	for (int row =startRow;row <= maxRow; ++row)
	{
		//读取两列的数据
		QVariant value1 = this->read(row, col1);
		QVariant value2 = this->read(row, col2);

		//交换数据
		this->write(row, col1, value2);
		this->write(row, col2, value1);
	}
}
...
QList<QList<QVariant>> QXlsx::Document::readExcelRange(int sRowIndex, int scolIndex, int eColIndex, const QString & filePath, const QString& sheetName)
{
	...
	for (int row = sRowIndex; row<= xlsx.dimension().lastRow();++row)
	{
		QList<QVariant> rowData;
		for (int col = scolIndex; col<=eColIndex;++col)
		{
			QXlsx::Cell *cell = xlsx.cellAt(row, col);
			if (cell)
			{
				rowData.append(cell->value().toString());
				auto temp = cell->formula().formulaText();
			if (cell->hasFormula())
			{
				auto formula =  cell->formula();
		//		formula.loadFromXml(xlsx);
				auto  temp1 = cell->readValue();
