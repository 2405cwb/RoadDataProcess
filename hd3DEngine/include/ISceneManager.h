...
namespace scene
{
	...
	*/
	...
	public:
		...
		//! Added by yaoli on 2.12/05/21
		//! Adds a customized style user controlled camera scene node to the scene graph.
		/** This is a standard camera with an animator that provides mouse control similar
		to camera in the 3D Software Maya by Alias Wavefront.
		\param parent: Parent scene node of the camera. Can be null.
		\param rotateSpeed: Rotation speed of the camera.
		...
		*/
		...
		//! 得到当前摄像机的animator; by yaoli 2012/05/25;
		virtual ISceneNodeAnimatorCameraCus* GetCameraSceneNodeAnimator(ESCENE_NODE_ANIMATOR_TYPE type) = 0;

		//! 根据屏幕坐标获取视椎体
		virtual void GetViewFrustum(const irr::core::recti& srcRect,SViewFrustum* pViewFrustum) = 0;

		virtual bool IsAnimating() = 0;

		virtual void SetAnimateState(bool bAnimating) = 0;

		// GetBuiltDefaultFont取代之 [zhangfei 2014/4/18]
		virtual gui::IGUIFont* GetBuiltInFont() = 0;

		virtual gui::IGUIFont* GetBuiltDefaultFont() = 0;

		virtual gui::IGUIFont* GetChineseFont() = 0;

		virtual gui::IGUIFont* GetAdaptiveFont(const char* srcStr, bool& bHaveWideChar) = 0;

		// 设置刷新渲染类型
		virtual void SetRefreshType(ESCENE_REFRESH_TYPE type){}
	    virtual ESCENE_REFRESH_TYPE GetRefreshType()=0;
	};
