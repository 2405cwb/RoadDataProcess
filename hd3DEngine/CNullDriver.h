...
namespace video
{
	...
	public:
		...
		//! 设置渐变色背景
		virtual void setGradientBackground(SColor color=SColor(255,0,0,0));
		...
		//! 绘制点集合-龚书林
		virtual void draw3DPointList(const void* vertices,u32 vertexCount,void* colorBuf,E_VERTEX_TYPE type);
		
		//! 绘制三维点-龚书林
		virtual void draw3DPoint(const core::vector3df& pt,SColor color = SColor(255,255,255,255));
		
		//! 绘制三维点-龚书林
		virtual void draw3DPoint(float x,float y,float z,SColor color = SColor(255,255,255,255));
		
		//! 设置绘制三维状态-龚书林
		virtual void setRenderStates3DMode();
		
		//! 绘制不规则二维多边形-汪开理
		virtual void draw2DPoly(bool &drawSuccess, const core::position2d<f32>* points2d, u32 vertexCount,SColor color = SColor(255,255,255,255));
		
		//! 绘制三角形-危迟
		virtual void draw2DTriangle(const core::position2d<f32>* points2d, u32 vertexCount,SColor color=SColor(255,255,255,255));
		
		//! 绘制球体-汪开理
		virtual void drawSphere(const core::vector3d<f32> center3d, f32 radius,SColor color = SColor(255,255,255,255));
		
		//! 绘制三维空间圆形-汪开理
		virtual void drawCircle(const core::vector3d<f32> center3d, f32 radius, const core::vector3d<f32> normal3d, u32 count, SColor color = SColor(255,255,255,255));
		
		//! 绘制二维圆形-冯晶
		virtual void draw2DCircle(const core::position2di src, int radius, SColor color = SColor(255,255,255,255));
		
		//! 绘制二维椭圆
		virtual void draw2DEllipse(const core::position2di src,int rx,int ry,SColor color = SColor(255,255,255,255));
		
		//! 绘制三维圆柱 危迟
		virtual void draw3DCylinder(const core::vector3d<f32> start,const core::vector3d<f32> end,f32 radius, SColor color = SColor(255,255,255,255));
		
		//绘制三维圆 [2013/09/23 危迟]
		virtual void draw3DCircle(const core::vector3d<f32> center3d, f32 radius, const core::vector3d<f32> normal3d, u32 count, SColor color = SColor(255,255,255,255));

		// 绘制三维圆弧(圆弧中心点，起始圆弧射线点，终止圆弧的射线点（射线点不需要在圆上）)-2014.04.11.zhubo
		virtual void draw3DArc(const core::vector3d<f32> center3d,
			const core::vector3d<f32> arcStart ,
			const core::vector3d<f32> arcEnd , 
			f32 radius, f32 fAngle,u32 count ,
			SColor color = SColor(255,255,255,255));

        // 绘制二维圆弧(圆弧中心点, 起始圆弧射线点，终止圆弧的射线点（射线点不需要在圆上）)-2016.04.07.张阳
        virtual void draw2DArc(const core::vector2di center2d,      // 圆弧所在圆的圆心
            f32 fBeginAngle,                                        // 圆弧初始角度
            f32 fEndAngle,                                          // 圆弧结束角度
            f32 radius,                                             // 圆弧半径
            u32 count,                                              // 圆弧实际是由多条线段构成，需要设置线段条数，使其更近似圆弧
            SColor color = SColor(255,255,255,255));                // 圆弧线段的颜色值
		
		// 绘制直角三角形垂线标记-2014.04.11 add by zhubo
		virtual void draw3DVerticalLine(const core::vector3d<f32> mid3d, // 垂足点
			const core::vector3d<f32> start3d,                           // 三角形顶点
			const core::vector3d<f32> end3d,                             // 三角形顶点
			f32 radius,                                                  // 垂足线段长度
			SColor color = SColor(255,255,255,255));

		// add by chy 2013/8/17
		virtual void DisplayAxis(core::vector3df center,   // data center
			...
			bool	bSolid = true);                    //  Display as Solid

		// 绘制带箭头的直线 [危迟 2015/11/19]
		...
		//! 用虚线绘制二维线段
		virtual void draw2dDashedLine(const core::position2d<s32>& start, 
			const core::position2d<s32>& end, SColor color=SColor(255,255,255,255));

        //! 用虚线绘制二维线段(兼容AMD显卡)
        virtual void draw2dDashedLineAMD(const core::position2d<s32>& start, 
            const core::position2d<s32>& end, SColor color=SColor(255,255,255,255));
	...
	protected:
		...
		struct SDummyTexture : public ITexture
		{
			...
			//! 获取对应的影像,gsl-2012/08/23
			virtual IImage* getImage() {return NULL;}
			//! 影像更新后,更新纹理
