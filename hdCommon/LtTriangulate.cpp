...
static int indexes[POINT_BRIO_BUFFER];
static int pointer;
					...
					int index0,int index1,int index2)
{
  ...
  if (orient > 0)
  {
	...
	// 指定索引值
	triangle_buffer[0].index[0] = index0;
	triangle_buffer[0].index[1] = index1;
	triangle_buffer[0].index[2] = index2;
  }
  else if (orient < 0)
  {
	...
	// 指定索引值
	triangle_buffer[0].index[0] = index1;
	triangle_buffer[0].index[1] = index0;
	triangle_buffer[0].index[2] = index2;
  }
  else
  {
  ...
  // 指定索引值
  triangle_buffer[1].index[0] = 0;
  triangle_buffer[1].index[1] = triangle_buffer[0].index[2];
  triangle_buffer[1].index[2] = triangle_buffer[0].index[1];
  ...
  // 指定索引值
  triangle_buffer[2].index[0] = 0;
  triangle_buffer[2].index[1] = triangle_buffer[0].index[0];
  triangle_buffer[2].index[2] = triangle_buffer[0].index[2];
  ...
  // 指定索引值
  triangle_buffer[3].index[0] = 0;
  triangle_buffer[3].index[1] = triangle_buffer[0].index[1];
  triangle_buffer[3].index[2] = triangle_buffer[0].index[0];
...
static void TINupdate(float* p, TINtriangle* t,int pIndex)
{
  ...
  while(!stkEMPTY())
  {
    ...
    else // we found a horizon edge. create the adjacent new triangle.
    {
      ...
      if (ci == 1)
      {
        ...
        tn->V[2] = p;
		// 指定索引值 
		tn->index[0] = t->index[0];
		tn->index[1] = t->index[2];  
		tn->index[2] = pIndex;
...
static bool TINinsert(float* p,int pindex)
{
...
bool TINclean(int num, bool convex_hull)
{
  ...
  if (triangle_buffer_alloc < num*2)
  {
    ...
    if (triangle_buffer == 0 || triangle_buffer_alloc > 300000000)
    {
		// 内存申请失败 将缓冲计数器设置为0 [2013/12/18 危迟]
  ...
  if (triangle_buffer)
  {	  
	  memset(triangle_buffer,0, triangle_buffer_alloc);
  ...
  if (initialized)
  {
    if (buffer_full)
    {
      if (pointer % 256 == 0)
      {
		...
		int helpIndex = indexes[pointer];
		indexes[pointer] = indexes[r];
		indexes[r] = helpIndex;
      }
      TINinsert(points[pointer],indexes[pointer]);
      points[pointer] = p;
  ...
  else
  {
    ...
    if (pointer >= 3)
    {
      if (TINinit(points[pointer-3], points[pointer-2], points[pointer-1],
				  indexes[pointer - 3],indexes[pointer - 2],indexes[pointer - 1]))
      {
        initialized = true;
        pointer -= 3;
      }
...
void TINfinish()
{
  ...
  if (buffer_full)
  {
    for (i = 0; i < POINT_BRIO_BUFFER; i++) TINinsert(points[i],indexes[i]);
  }
  else
  {
    for (i = 0; i < pointer; i++) TINinsert(points[i],indexes[i]);
  }
}
