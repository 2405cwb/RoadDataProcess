...
#ifndef _HD_PLANE3D_H__
#define _HD_PLANE3D_H__
...
class plane3d
{
	public:
		...
		plane3d(): Normal(0,1,0) { recalculateD(CHdVector3d<T>(0,0,0)); }
		
		plane3d(const CHdVector3d<T>& MPoint, const CHdVector3d<T>& Normal) : Normal(Normal) { recalculateD(MPoint); }
		
		plane3d(T px, T py, T pz, T nx, T ny, T nz) : Normal(nx, ny, nz) { recalculateD(CHdVector3d<T>(px, py, pz)); }
		
		plane3d(const CHdVector3d<T>& point1, const CHdVector3d<T>& point2, const CHdVector3d<T>& point3)
		{ setPlane(point1, point2, point3); }
		
		plane3d(const CHdVector3d<T> & normal, const T d) : Normal(normal), D(d) { }
		...
		void setPlane(const CHdVector3d<T>& point, const CHdVector3d<T>& nvector)
		{
			Normal = nvector;
			recalculateD(point);
		}

		void setPlane(const CHdVector3d<T>& nvect, T d)
		{
			Normal = nvect;
			D = d;
		}

		void setPlane(const CHdVector3d<T>& point1, const CHdVector3d<T>& point2, const CHdVector3d<T>& point3)
		{
		...
		*/
		bool getIntersectionWithLine(const CHdVector3d<T>& linePoint,
				const CHdVector3d<T>& lineVect,
				CHdVector3d<T>& outIntersection) const
		{
		...
		*/
		f32 getKnownIntersectionWithLine(const CHdVector3d<T>& linePoint1,
			const CHdVector3d<T>& linePoint2) const
		{
			CHdVector3d<T> vect = linePoint2 - linePoint1;
		...
		*/
		bool getIntersectionWithLimitedLine(
				const CHdVector3d<T>& linePoint1,
				const CHdVector3d<T>& linePoint2,
				CHdVector3d<T>& outIntersection) const
		{
		...
		EIntersectionRelation3D classifyPointRelation(const CHdVector3d<T>& point) const
		{
		...
		void recalculateD(const CHdVector3d<T>& MPoint)
		{
			D = - MPoint.dotProduct(Normal);
		}

		//! Gets a member point of the plane.
		CHdVector3d<T> getMemberPoint() const
		{
			return Normal * -D;
		}
		...
		{
			CHdVector3d<T> cross = other.Normal.crossProduct(Normal);
		...
		bool getIntersectionWithPlane(const plane3d<T>& other,
				CHdVector3d<T>& outLinePoint,
				CHdVector3d<T>& outLineVect) const
		{
		...
		bool getIntersectionWithPlanes(const plane3d<T>& o1,
				const plane3d<T>& o2, CHdVector3d<T>& outPoint) const
		{
			CHdVector3d<T> linePoint, lineVect;
		...
		bool isFrontFacing(const CHdVector3d<T>& lookDirection) const
		{
		...
		/** Note that this only works if the normal is normalized. */
		T getDistanceTo(const CHdVector3d<T>& point) const
		{
			return point.dotProduct(Normal) + D;
		}

		//! Normal vector of the plane.
		CHdVector3d<T> Normal;
