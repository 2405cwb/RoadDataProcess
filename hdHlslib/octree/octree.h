...
namespace hd
{
	...
	{
	public:
		BlockPos3D(){x = 0; y = 0; z = 0;}
		BlockPos3D(int nx, int ny, int nz){x = nx; y = ny; z = nz;}
	public:
		int x;
		int y;
		int z;
	...
	template< typename T, int AS = 1/*2*/ >
	class Octree
	{
	public:
		//Octree();
	...
	public:
		...
		// 扩展接口:遍历叶子节点[zfei 2014/7/29]
		int SearchLeafNodes( Node* pNode, std::vector<T>& vecNodes);
	 ...
	 */
	 ...
	 Octree<T,AS>::Octree(const T& emptyValue = T(0)) : root_(0), emptyValue_(emptyValue)
	 {
	 ...
	 template< typename T, int AS >
	 void Octree<T,AS>::setSize(int nSize)
	 {
	 ...
	 */
	template< typename T, int AS >
	Octree<T,AS>::Octree( const Octree<T,AS>& o ) : emptyValue_(o.emptyValue_), size_( o.size_ )
	{
		if ( !o.root_ )
		{
			root_ = 0;
	...
	void Octree<T,AS>::zSliceRecursive(Array2D<T> slice, const Node* node,int size, int x, int y, int z, int targetZ) const
	{
		...
		else if ( node->type() == BranchNode )
		{
			...
			for ( int i = 0; i < 2; ++i )
			{
				for ( int j = 0; j < 2; ++j )
				{
					zSliceRecursive( slice.subarray(i*size, j*size, (i+1)*size, (j+1)*size), reinterpret_cast<const Branch*>(node)->child(
	...
	// 扩展接口，获取叶子节点
	template< typename T, int AS >
	int Octree<T,AS>::SearchLeafNodes( Node* pNode, std::vector<T>& vecNodes)
	{
		if (pNode == NULL)
		{
			return FALSE;
		}
		if (pNode->type() == BranchNode)		// 遍历“分支”节点的八个子节点
		{
			Branch* pCurb = reinterpret_cast<Branch*>(pNode);
			if (pCurb == NULL)
			{
				return FALSE;
			}
			for (int i = 0; i < 8; i++)
			{
				return SearchLeafNodes(pCurb->child(i), vecNodes);
			}
		}
		else if (pNode->type() == AggregateNode)	// 遍历"集合"节点的AS*AS*AS子节点
		{
			for (int i = 0; i < AS; ++i)
			{
				for (int j = 0; j < AS; ++j)
				{
					for (int k = 0; k < AS; ++k)
					{
						const T& val = (reinterpret_cast<Aggregate*>(pNode))->value(k,j,i);
						if (val != emptyValue_)
						{
							vecNodes.push_back(val);
						}
		...
		else if (pNode->type() == LeafNode)		// 遍历到叶子节点
		{
			const T& val = (reinterpret_cast<Leaf*>(pNode))->value();
			if (val != emptyValue_)
			{
				vecNodes.push_back(val);
			}
