	...
	ReadOnlyLock(false), KeepImage(true)
{
	...
	bool bRet = uploadTexture(true, mipmapData);
	if (!bRet && Image)
	{
		// 创建失败,释放图片对象
		Image->drop();
...
bool COpenGLTexture::update()
{
	//return uploadTexture(true, 0);
	return uploadTexture(false,0);
}
...
void COpenGLTexture::getImageValues(IImage* image)
{
	...
	if ((ImageSize.Width>Driver->MaxTextureSize) && (ratio >= 1.0f))
	{
		...
		TextureSize.Width = Driver->MaxTextureSize;
		TextureSize.Height = (u32)(Driver->MaxTextureSize/ratio);
		TextureSize = TextureSize.getOptimalSize(!Driver->queryFeature(EVDF_TEXTURE_NPOT));
	}
	else if (ImageSize.Height>Driver->MaxTextureSize)
	{
		...
		TextureSize.Height = Driver->MaxTextureSize;
		TextureSize.Width = (u32)(Driver->MaxTextureSize*ratio);
		TextureSize = TextureSize.getOptimalSize(!Driver->queryFeature(EVDF_TEXTURE_NPOT));
	}
	//TextureSize=ImageSize.getOptimalSize(!Driver->queryFeature(EVDF_TEXTURE_NPOT));
...
bool COpenGLTexture::uploadTexture(bool newTexture, void* mipmapData, u32 level)
{
