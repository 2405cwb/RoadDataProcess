...
*****/
class hnFramelessHelperPrivate
{
public:
    QHash<QWidget*, hnWidgetData*> m_widgetDataHash;
...
*****/
class hnCursorPosCalculator
{
public:
    explicit hnCursorPosCalculator();
    void reset();
...
int hnCursorPosCalculator::m_nBorderWidth = 5;
int hnCursorPosCalculator::m_nTitleHeight = 30;

/***** CursorPosCalculator *****/
hnCursorPosCalculator::hnCursorPosCalculator()
{
    reset();
}

void hnCursorPosCalculator::reset()
{
    m_bOnEdges = false;
    m_bOnLeftEdge = false;
...
void hnCursorPosCalculator::recalculate(const QPoint &gMousePos, const QRect &frameRect)
{
...
{
public:
    explicit hnWidgetData(hnFramelessHelperPrivate *d, QWidget *pTopLevelWidget);
    ~hnWidgetData();
    QWidget* widget();
    // 处理鼠标事件-划过、按下、释放、移动
...
private:
    hnFramelessHelperPrivate *d;
    QRubberBand *m_pRubberBand;
    QWidget *m_pWidget;
    QPoint m_ptDragPos;
	hnCursorPosCalculator m_pressedMousePos;
	hnCursorPosCalculator m_moveMousePos;
...
/***** WidgetData *****/
hnWidgetData::hnWidgetData(hnFramelessHelperPrivate *_d, QWidget *pTopLevelWidget)
{
    d = _d;
...
QWidget* hnWidgetData::widget()
{
    return m_pWidget;
}

bool hnWidgetData::handleWidgetEvent(QEvent *event)
{
    switch (event->type())
    {
...
void hnWidgetData::updateRubberBandStatus()
{
...
void hnWidgetData::updateCursorShape(const QPoint &gMousePos)
{
...
void hnWidgetData::resizeWidget(const QPoint &gMousePos)
{
    QRect origRect;
...
void hnWidgetData::moveWidget(const QPoint& gMousePos)
{
...
bool hnWidgetData::handleMousePressEvent(QMouseEvent *event)
{
...
bool hnWidgetData::handleMouseReleaseEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton)
    {
        m_bLeftButtonPressed = false;
...
bool hnWidgetData::handleMouseMoveEvent(QMouseEvent *event)
{
...
bool hnWidgetData::handleLeaveEvent(QEvent *event)
{
    Q_UNUSED(event)
    if (!m_bLeftButtonPressed)
    {
...
bool hnWidgetData::handleHoverMoveEvent(QHoverEvent *event)
{
...
bool hnWidgetData::handleDoubleClickedMouseEvent(QMouseEvent *event)
{
    if(event->button() == Qt::LeftButton)
    {
        if(m_pWidget)
...
hnFramelessHelper::hnFramelessHelper(QObject *parent)
    : QObject(parent),
      d(new hnFramelessHelperPrivate())
{
...
hnFramelessHelper::~hnFramelessHelper()
{
...
bool hnFramelessHelper::eventFilter(QObject *obj, QEvent *event)
{
    switch (event->type())
    {
    case QEvent::MouseMove:
    ...
    case QEvent::Leave:
    {
		hnWidgetData *data = d->m_widgetDataHash.value(static_cast<QWidget*>(obj));
        if (data)
        {
...
void hnFramelessHelper::activateOn(QWidget *topLevelWidget)
{
    if (!d->m_widgetDataHash.contains(topLevelWidget))
    {
		hnWidgetData *data = new hnWidgetData(d, topLevelWidget);
...
void hnFramelessHelper::removeFrom(QWidget *topLevelWidget)
{
	hnWidgetData *data = d->m_widgetDataHash.take(topLevelWidget);
    if (data)
    {
...
void hnFramelessHelper::setRubberBandOnMove(bool movable)
{
    d->m_bRubberBandOnMove = movable;
    QList<hnWidgetData*> list = d->m_widgetDataHash.values();
    foreach (hnWidgetData *data, list)
    {
        data->updateRubberBandStatus();
    }
}

void hnFramelessHelper::setWidgetMovable(bool movable)
{
    d->m_bWidgetMovable = movable;
}

void hnFramelessHelper::setWidgetResizable(bool resizable)
{
    d->m_bWidgetResizable = resizable;
}

void hnFramelessHelper::setRubberBandOnResize(bool resizable)
{
...
void hnFramelessHelper::setBorderWidth(uint width)
{
    if (width > 0)
    {
		hnCursorPosCalculator::m_nBorderWidth = width;
    }
}

void hnFramelessHelper::setTitleHeight(uint height)
{
    if (height > 0)
    {
		hnCursorPosCalculator::m_nTitleHeight = height;
    }
}

bool hnFramelessHelper::widgetMovable()
{
    return d->m_bWidgetMovable;
}

bool hnFramelessHelper::widgetResizable()
{
    return d->m_bWidgetResizable;
}

bool hnFramelessHelper::rubberBandOnMove()
{
    return d->m_bRubberBandOnMove;
}

bool hnFramelessHelper::rubberBandOnResisze()
{
    return d->m_bRubberBandOnResize;
}

uint hnFramelessHelper::borderWidth()
{
    return hnCursorPosCalculator::m_nBorderWidth;
}

uint hnFramelessHelper::titleHeight()
{
    return hnCursorPosCalculator::m_nTitleHeight;
}
