...
class CHdBox3d
{
	public:
		...
		//! Default Constructor.
		CHdBox3d(): MinEdge(-1,-1,-1), MaxEdge(1,1,1) {}
		//! Constructor with min edge and max edge.
		CHdBox3d(const CHdVector3d<T>& min, const CHdVector3d<T>& max): MinEdge(min), MaxEdge(max) {}
		//! Constructor with only one point.
		CHdBox3d(const CHdVector3d<T>& init): MinEdge(init), MaxEdge(init) {}
		//! Constructor with min edge and max edge as single values, not vectors.
		CHdBox3d(T minx, T miny, T minz, T maxx, T maxy, T maxz): MinEdge(minx, miny, minz), MaxEdge(maxx, maxy, maxz) {}

		// operators
		//! Equality operator
		/** \param other box to compare with.
		\return True if both boxes are equal, else false. */
		inline bool operator==(const CHdBox3d<T>& other) const { return (MinEdge == other.MinEdge && other.MaxEdge == MaxEdge);}
		//! Inequality operator
		/** \param other box to compare with.
		...
		void reset(const CHdBox3d<T>& initValue)
		{
			*this = initValue;
		}
		...
		void reset(const CHdVector3d<T>& initValue)
		{
			MaxEdge = initValue;
			MinEdge = initValue;
		}
		...
		void addInternalPoint(const CHdVector3d<T>& p)
		{
		...
		void addInternalBox(const CHdBox3d<T>& b)
		{
		...
		T getVolume() const
		{
			const CHdVector3d<T> e = getExtent();
			return e.X * e.Y * e.Z;
		}
		...
		void getEdges(CHdVector3d<T> *edges) const
		{
			const core::CHdVector3d<T> middle = getCenter();
			const core::CHdVector3d<T> diag = middle - MaxEdge;
		...
		CHdBox3d<T> getInterpolated(const CHdBox3d<T>& other, f32 d) const
		{
			f32 inv = 1.0f - d;
			return CHdBox3d<T>((other.MinEdge*inv) + (MinEdge*d),
		...
		bool isPointInside(const CHdVector3d<T>& p) const
		{
		...
		\param p: Point to check.
		\return True if the point is within the box and false if not. */
		bool isPointTotalInside(const CHdVector3d<T>& p) const
		{
			return (p.X > MinEdge.X && p.X < MaxEdge.X &&
				p.Y > MinEdge.Y && p.Y < MaxEdge.Y &&
				p.Z > MinEdge.Z && p.Z < MaxEdge.Z);
		...
		otherwise false. */
		bool isFullInside(const CHdBox3d<T>& other) const
		{
		...
		otherwise false. */
		bool intersectsWithBox(const CHdBox3d<T>& other) const
		{
		...
		bool intersectsWithBox(T minx,T miny,T minz,T maxx,T maxy,T maxz) const
		{
			return (MinEdge.X <= maxx && MinEdge.Y <= maxy && MinEdge.Z <= maxz &&
				MaxEdge.X >= minx && MaxEdge.Y >= miny && MaxEdge.Z >= minz);
		}
		...
		bool intersectsWithLine(const CHdVector3d<T>& linemiddle,
					const CHdVector3d<T>& linevect, T halflength) const
		{
			const CHdVector3d<T> e = getExtent() * (T)0.5;
			const CHdVector3d<T> t = getCenter() - linemiddle;
		...
		{
			CHdVector3d<T> nearPoint(MaxEdge);
			CHdVector3d<T> farPoint(MinEdge);
		...
		//! The near edge
		CHdVector3d<T> MinEdge;

		//! The far edge
		CHdVector3d<T> MaxEdge;
};

	//! Typedef for a f32 3d bounding box.
	typedef CHdBox3d<float> CHdBox3df;
	//! Typedef for a f64 3d bounding box.
	typedef CHdBox3d<double> CHdBox3dd;
	//! Typedef for an integer 3d bounding box.
	typedef CHdBox3d<int> CHdBox3di;
