...
ISceneNode* CSceneCollisionManager::getSceneNodeFromRayBB( 
	const core::line3d<f32>& ray, core::array<ESCENE_NODE_TYPE> TypeList,
	bool bNoDebugObjects, ISceneNode* root )
{
	ISceneNode* best = 0;
	f32 dist = FLT_MAX;
	...
	getPickedNodeBB((root==0)?SceneManager->getRootSceneNode():root, truncatableRay,
		TypeList, bNoDebugObjects, dist, best);

	return best;
}

void CSceneCollisionManager::getSceneNodesFromRayBB(const core::line3d<f32>& ray,
	core::array<ISceneNode*>& outNodes,
	s32 idBitMask, bool bNoDebugObjects, ISceneNode* root)
{
	core::line3d<f32> truncatableRay(ray);

	getPickedNodesBB((root==0)?SceneManager->getRootSceneNode():root, truncatableRay,
		idBitMask, bNoDebugObjects, outNodes);

}

void CSceneCollisionManager::getPickedNodesBB( ISceneNode* root, core::line3df& ray,
	s32 bits, bool bNoDebugObjects, core::array<ISceneNode*>& outNodes)
{
	...
	for (; it != children.end(); ++it)
	{
		...
		if (current->isVisible())
		{
			...
			getPickedNodesBB(current, ray, bits, bNoDebugObjects, outNodes);
		}
	}
}
...
void CSceneCollisionManager::getPickedNodeBB( ISceneNode* root, core::line3df& ray,
	core::array<ESCENE_NODE_TYPE> typelist, bool bNoDebugObjects, 
	f32& outbestdistance, ISceneNode*& outbestnode )
{
	const ISceneNodeList& children = root->getChildren();
	const core::vector3df rayVector = ray.getVector().normalize();
	...
	for (; it != children.end(); ++it)
	{
		...
		if (current->isVisible())
		{
			...
			if((bNoDebugObjects ? !current->isDebugObject() : true) && bRightType)
			{
				// get world to object space transform
				core::matrix4 worldToObject;
				if (!current->getAbsoluteTransformation().getInverse(worldToObject))
			...
			// Only check the children if this node is visible.
			getPickedNodeBB(current, ray, typelist, bNoDebugObjects, outbestdistance, outbestnode);
		}
	}
}
...
//! Calculates 2d screen position from a 3d position.
core::position2d<s32> CSceneCollisionManager::getScreenCoordinatesFrom3DPosition(
    const core::vector3dd & pos3d, ICameraSceneNode* camera)
{
    if (!SceneManager || !Driver)
        return core::position2d<s32>(-1000,-1000);
...
bool CSceneCollisionManager::get3DPositionFromScreenPos( u32 srcX,u32 srcY,float& x,float& y,float& z )
{
	if (!SceneManager || !Driver)
		return false;
	...
	if (srcX < 0 || srcX >= dim.Width 
		|| srcY < 0 || srcY >= dim.Height)
	{
		return false;
	}

	core::matrix4 trans = camera->getProjectionMatrix();
	core::matrix4 view = camera->getViewMatrix();
	
	GLint viewport[4];
	GLdouble modelview[16];
	GLdouble projection[16];
	GLfloat winX,winY,winZ;
	GLdouble posX,posY,posZ = 0;

	for (int i = 0;i<16;i++)
	{
		modelview[i] = view[i];
		projection[i] = trans[i];
	}

	glGetIntegerv(GL_VIEWPORT,viewport);
	
	winX=(float)srcX;
	winY=viewport[3]-(float)srcY;
...
bool CSceneCollisionManager::get3DPositionFromScreenPos( u32 srcX,u32 srcY,core::vector3df& position )
{
	return get3DPositionFromScreenPos(srcX,srcY,position.X,position.Y,position.Z);
}

bool  CSceneCollisionManager::GetWorldCoord(int ix, int iy,double fz, core::vector3df& coord)
{
...
void CSceneCollisionManager::projectPointPixels(float x,float y,float z, float &proj_x_px, float &proj_y_px, float &proj_z_depth)
{
	if (!SceneManager || !Driver)
	{
		proj_x_px = -1000.f;
	...
	proj_x_px = dim.Width * transformedPos[0] * zDiv + dim.Width;
	proj_y_px  = dim.Height - dim.Height * transformedPos[1] * zDiv;
	proj_z_depth = transformedPos[2];
