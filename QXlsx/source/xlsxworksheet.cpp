...
QVariant Worksheet::read(int row, int column) const
{
 ...
 QVariant QXlsx::Worksheet::readCellValue(int row, int column) const
{
	 Q_D(const Worksheet);
	 ...
	 if (cell->hasFormula())
	 {
		 ...
		 QString temp = cell->formula().formulaText();
#ifdef DEBUG
		 qDebug() <<  QString::fromLocal8Bit ("¹«Ê½");
		 qDebug() << "cell->formula().isValid" << cell->formula().isValid();
		 qDebug() << "cell->formula().formulaText" << cell->formula().formulaText();
		 qDebug() << "cell->formula().formulaTYpe" << cell->formula().formulaType();
		 qDebug() << "cell->value" << cell->value();
#endif // DEBUG

 
		 if (v.toString().compare(emptyVariant.toString()) == 0 )
		 {
