	...
	public:
		...
		void Clear()
		{
			...
			if(m_pLoopSeg)
			{
				...
				operator delete((void*)m_pLoopSeg);//delete[] m_pLoopSeg;
				m_pLoopSeg = NULL;
			}
		...
		void AllocLoopSeg()
		{
			if(m_loopIdx.segCount > 0)
			{
				...
				U32 byteCount = m_loopIdx.segCount * sizeof(HLS2_LOOPSEG);
				m_pLoopSeg = (HLS2_LOOPSEG*)(operator new(byteCount));
				memset(m_pLoopSeg,0,byteCount);
			}
		}
		// 圈索引
