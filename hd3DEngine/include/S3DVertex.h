...
//! 
struct S3DVertexXYZIRGBA : public S3DVertex
{
	//! default constructor
	S3DVertexXYZIRGBA() : S3DVertex() { }

	//! constructor
	S3DVertexXYZIRGBA(f32 x, f32 y, f32 z, f32 nx=0.0f, f32 ny=0.0f, f32 nz=0.0f,
		SColor c = 0xFFFFFFFF, f32 tu=0.0f, f32 tv=0.0f,
		u16 intens = 0)
		: S3DVertex(x,y,z, nx,ny,nz, c, tu,tv), intensity(intensity) { }

	//! constructor
	S3DVertexXYZIRGBA(const core::vector3df& pos, SColor c,
		const core::vector2df& tcoords)
		: S3DVertex(pos, core::vector3df(), c, tcoords) { }

	//! constructor
	S3DVertexXYZIRGBA(const core::vector3df& pos,
		const core::vector3df& normal, SColor c,
		const core::vector2df& tcoords,
		const u16& intens
		)
		: S3DVertex(pos, normal, c, tcoords), intensity(intens){ }

	u16 intensity;

	bool operator==(const S3DVertexXYZIRGBA& other) const
	{
		return ((static_cast<S3DVertex>(*this)==other) &&
	...
	{
		return ((static_cast<S3DVertex>(*this) < other) ||
			((static_cast<S3DVertex>(*this) == other) && (intensity < other.intensity)));
	}
...
inline u32 getVertexPitchFromType(E_VERTEX_TYPE vertexType)
{
	...
	case  video::EVT_XYZFLOAT:
		return sizeof(video::S3DVertexXYZIRGBA);
	default:
