...
IMesh* CGeometryCreator::createSphereMesh(f32 radius, u32 polyCountX, u32 polyCountY) const
{
	...
	// 设置创建的轨迹球颜色为红色 【2013-09-13 危迟】
	const video::SColor clr(100, 255,0,0);
...
/* A sphere with proper normals and texture coords */ //add color [add by mzm 2013.10.17]
IMesh* CGeometryCreator::createSphereMesh(f32 radius, u32 polyCountX, u32 polyCountY, video::SColor sclr) const
{
	// thanks to Alfaz93 who made his code available for Irrlicht on which
	// this one is based!
...
//添加planmesh,add by mzm
IMesh* CGeometryCreator::createPlanMesh(const core::vector3df& size, video::SColor sclr) const
{
	SMeshBuffer* Buffer = new SMeshBuffer();
	...
	Buffer->BoundingBox.MinEdge.set(-halfW, -0.01f, -halfH);
	Buffer->BoundingBox.MaxEdge.set(halfW,  0.01f ,halfH);
	...
	video::S3DVertex vtx;
	vtx.Color.set(sclr.getAlpha(),sclr.getRed(),sclr.getGreen(),sclr.getBlue());
	vtx.Normal.set(0.0f,-1.0f,0.0f);

	vtx.Pos.set(-halfW, 0.0f, -halfH);
