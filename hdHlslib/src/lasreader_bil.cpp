...
BOOL LASreaderBIL::read_hdr_file(const CHAR* file_name)
{
  ...
  while (fgets(line, 256, file))
  {
    ...
    {
      sscanf_s(line, "%s %d", dummy, &nbands);
    }
    else if (strstr(line, "nbits") || strstr(line, "NBITS"))
    {
      sscanf_s(line, "%s %d", dummy, &nbits);
    }
    else if (strstr(line, "layout") || strstr(line, "LAYOUT"))
    {
      CHAR layout[32];
      sscanf_s(line, "%s %s", dummy, layout);
    ...
    {
      CHAR pixeltype[32];
      sscanf_s(line, "%s %s", dummy, pixeltype);
    ...
    {
      CHAR byteorder[32];
      sscanf_s(line, "%s %s", dummy, byteorder);
...
BOOL LASreaderBIL::read_blw_file(const CHAR* file_name)
{
  ...
  sscanf_s(line, "%lf", &ulxcenter);
  if (!fgets(line, 256, file))
  {
    fprintf(stderr, "WARNING: corrupt world file\n");
    return FALSE;
  }
  sscanf_s(line, "%lf", &ulycenter);
  return TRUE;
}
