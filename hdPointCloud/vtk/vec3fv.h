...
inline void VecAddOffset3fv(double v[3], const double offset[3], const float a[3]);
inline void VecSubtractOffset3fv(float v[3], const double offset[3], const double a[3]);

inline void VecScale3fv(float v[3], const float scale[3], const int a[3]);
inline void VecScale3fv(int v[3], const float scale[3], const float a[3]);
...
inline void VecAddOffset3fv(double v[3], const double offset[3], const float a[3])
{
  v[0] = (double)(a[0]) + offset[0];
  v[1] = (double)(a[1]) + offset[1];
  v[2] = (double)(a[2]) + offset[2];
}

inline void VecSubtractOffset3fv(float v[3], const double offset[3], const double a[3])
{
  v[0] = (float)(a[0] - offset[0]);
  v[1] = (float)(a[1] - offset[1]);
  v[2] = (float)(a[2] - offset[2]);
}

inline void VecScale3fv(float v[3], const float scale[3], const int a[3])
{
  v[0] = scale[0] * (float)(a[0]);
  v[1] = scale[1] * (float)(a[1]);
  v[2] = scale[2] * (float)(a[2]);
}

inline void VecScale3fv(int v[3], const float scale[3], const float a[3])
{
  v[0] = (int)((a[0] / scale[0]) + 0.5f);
  v[1] = (int)((a[1] / scale[1]) + 0.5f);
  v[2] = (int)((a[2] / scale[2]) + 0.5f);
}
