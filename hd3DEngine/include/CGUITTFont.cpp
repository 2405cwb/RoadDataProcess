...
CUserStringConvert::CUserStringConvert()
...
char* CUserStringConvert::WcharToChar(const wchar_t* wc)
{
	ReleaseResource();
	int nLen = WideCharToMultiByte(CP_ACP, 0, wc, wcslen(wc), NULL, 0, NULL, NULL);

	m_char = new char[nLen + 1];
	WideCharToMultiByte(CP_ACP, 0, wc, wcslen(wc), m_char, nLen, NULL, NULL);
	m_char[nLen] = '\0';

	return m_char;
}

wchar_t* CUserStringConvert::CharToWchar(const char* c)
{
	ReleaseResource();
	int nLen = MultiByteToWideChar(CP_ACP, 0, c,strlen(c), NULL, 0);
	m_wchar = new wchar_t[nLen + 1];
	MultiByteToWideChar(CP_ACP, 0, c, strlen(c), m_wchar, nLen);
	m_wchar[nLen] = '\0';
	
	return m_wchar;
}

void CUserStringConvert::ReleaseResource()
{
	if (m_char)
	{
		delete m_char;
...
void CGUITTFont::SetFontSize(EHD_FONT_SIZE nSize)
{
	// 字号相同则不需再进行设置
	if (size == nSize)	
	{
...
wchar_t* CGUITTFont::CharToWchar(const char* c)
{
	return m_StringConvert.CharToWchar(c);
}

bool CGUITTFont::IsExistWideChar(const char* str)
{
	// 判断宽字节的方法为：相邻两个字符都大于0x80
	char c;
	while (1)
	{
		c = *str++;
