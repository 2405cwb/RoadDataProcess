...
class HDPOINTCLOUD_API PointCloud2 //class HDCOMMON_API
{
...
private:
	//! 禁止拷贝
	PointCloud2(const PointCloud2 &pcd)
	{
	}
	//! 禁止复制
	const PointCloud2& operator=(const PointCloud2& other)
	{
		return *this;
	}
	//! 获取当前读取的点,坐标系是本地坐标
...
public:
	...
	PointCloud2():m_hlsReader(0),m_pIndex(0),m_bChange(false),m_maxIntensity(0),m_minIntensity(65536),
		...
		m_startScale(0.0),m_endScale(1.0),m_loadSimple(5000000),m_curRoop(0){}
	...
	BOOL open()
	{
		...
		if(m_hlsReader)
		{
			bRet = m_hlsReader->Open(m_hlsReader->GetFilePath());
