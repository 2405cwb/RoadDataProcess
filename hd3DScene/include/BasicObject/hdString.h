...
template <typename T, typename TAlloc = CHdAllocator<T> >
class hdstring
{
public:
	...
	//! Constructor
	hdstring(const hdstring<T,TAlloc>& other)
	...
	hdstring(const hdstring<B, A>& other)
	: array(0), allocated(0), used(0)
	{
		*this = other;
	}
	...
	explicit hdstring(const double number)
	: array(0), allocated(0), used(0)
	{
		c8 tmpbuf[255];
	...
	explicit hdstring(unsigned int number)
	: array(0), allocated(0), used(0)
	{
		// temporary buffer for 16 numbers
	...
	template <class B>
	hdstring(const B* const c, u32 length)
	: array(0), allocated(0), used(0)
	{
		if (!c)
		{
	...
	//! Assignment operator
	hdstring<T,TAlloc>& operator=(const hdstring<T,TAlloc>& other)
	{
		if (this == &other)
			return *this;
	...
	template <class B, class A>
	hdstring<T,TAlloc>& operator=(const hdstring<B,A>& other)
	{
		*this = other.c_str();
		return *this;
	}
	...
	template <class B>
	hdstring<T,TAlloc>& operator=(const B* const c)
	{
		if (!c)
		{
			if (!array)
	...
	hdstring<T,TAlloc> operator+(const hdstring<T,TAlloc>& other) const
	{
		hdstring<T,TAlloc> str(*this);
		str.append(other);
	...
	template <class B>
	hdstring<T,TAlloc> operator+(const B* const c) const
	{
		hdstring<T,TAlloc> str(*this);
		str.append(c);
	...
	{
		_HD_DEBUG_BREAK_IF(index>=used) // bad index
		return array[index];
	}
	...
	//! Equality operator
	bool operator ==(const hdstring<T,TAlloc>& other) const
	{
	...
	bool equals_ignore_case(const hdstring<T,TAlloc>& other) const
	{
	...
	\return True if the strings are equal ignoring case. */
	bool equals_substring_ignore_case(const hdstring<T,TAlloc>&other, const s32 sourcePos = 0 ) const
	{
	...
	bool lower_ignore_case(const hdstring<T,TAlloc>& other) const
	{
		for(u32 i=0; array[i] && other.array[i]; ++i)
		{
	...
	bool equalsn(const hdstring<T,TAlloc>& other, u32 n) const
	{
		u32 i;
	...
	void append(const hdstring<T,TAlloc>& other)
	{
		--used;
	...
	void append(const hdstring<T,TAlloc>& other, u32 length)
	{
	...
	{
		start = hd::clamp ( start < 0 ? (s32)(used) - 1 : start, 0, (s32)(used) - 1 );
	...
	hdstring<T,TAlloc> subString(u32 begin, s32 length) const
	{
		// if start after string
		// or no proper substring length
		if ((length <= 0) || (begin>=size()))
			return hdstring<T,TAlloc>("");
	...
	hdstring<T,TAlloc>& operator += (T c)
	{
		append(c);
		return *this;
	}
	...
	hdstring<T,TAlloc>& operator += (const T* const c)
	{
		append(c);
		return *this;
	}
	...
	hdstring<T,TAlloc>& operator += (const int i)
	{
		append(hdstring<T,TAlloc>(i));
		return *this;
	}
	...
	//! Appends a string representation of a number to this string
	/** \param i Number to append. */
	hdstring<T,TAlloc>& operator += (const unsigned int i)
	{
		append(hdstring<T,TAlloc>(i));
		return *this;
	}
	...
	//! Appends a string representation of a number to this string
	/** \param i Number to append. */
	hdstring<T,TAlloc>& operator += (const long i)
	{
		append(hdstring<T,TAlloc>(i));
		return *this;
	}
	...
	//! Appends a string representation of a number to this string
	/** \param i Number to append. */
	hdstring<T,TAlloc>& operator += (const unsigned long& i)
	{
		append(hdstring<T,TAlloc>(i));
		return *this;
	}
	...
	//! Appends a string representation of a number to this string
	/** \param i Number to append. */
	hdstring<T,TAlloc>& operator += (const double i)
	{
		append(hdstring<T,TAlloc>(i));
		return *this;
	}
	...
	//! Appends a string representation of a number to this string
	/** \param i Number to append. */
	hdstring<T,TAlloc>& operator += (const float i)
	{
		append(hdstring<T,TAlloc>(i));
		return *this;
	}
	...
	void remove(const hdstring<T,TAlloc> toRemove)
	{
	...
	void removeChars(const hdstring<T,TAlloc> & characters)
	{
		u32 pos = 0;
		u32 found = 0;
		for (u32 i=0; i<used; ++i)
	...
	hdstring<T,TAlloc>& trim(const hdstring<T,TAlloc> & whitespace = " \t\n\r")
	{
	...
	*/
	...
	u32 split(container& ret, const T* const c, u32 count=1, bool ignoreEmptyTokens=true, bool keepSeparators=false) const
	{
			...
			for (u32 j=0; j<count; ++j)
			{
				if (array[i] == c[j])
				{
					if ((!ignoreEmptyTokens || i - lastpos != 0) &&
							!lastWasSeparator)
						ret.push_back(hdstring<T,TAlloc>(&array[lastpos], i - lastpos));
					foundSeparator = true;
		...
		if ((used - 1) > lastpos)
			ret.push_back(hdstring<T,TAlloc>(&array[lastpos], (used - 1) - lastpos));
...
typedef hdstring<char> stringc;

//! Typedef for wide character strings
typedef hdstring<wchar_t> stringw;

//! ×Ö·û´®Ìæ»»,gsl-2013/09/23
