...
hn2DCameraView::hn2DCameraView(QWidget *parent)//m_scale(0.25)
	...
	, m_dFirstScale(0.14),m_bShow(true),m_nViewID(1)
{
	...
	// TEST
	m_operateBar = new QToolBar("", this);

	//addToolBar(m_operateBar);

	// 创建打开工程文件
	const QIcon zfIcon = QIcon::fromTheme("docment-pcData", QIcon(QStringLiteral(":/Resources/icons/28-打开工程.png")));
	m_preImageAct = new QAction(zfIcon, QStringLiteral("&打开工程"), this);
	m_operateBar->addAction(m_preImageAct);
}

hn2DCameraView::~hn2DCameraView()
{
	if (m_LoadPic)
	{
		delete m_LoadPic;
...
// 绘制事件
void hn2DCameraView::paintEvent(QPaintEvent *)
{
...
// 窗体大小改变事件
void hn2DCameraView::resizeEvent(QResizeEvent * e)
{
...
// 鼠标滚动事件(放大缩小)
void hn2DCameraView::wheelEvent(QWheelEvent *e)
{
	if (!m_LoadPic)
	{
		return;
...
// 鼠标按下事件
void hn2DCameraView::mousePressEvent(QMouseEvent *event)
{
...
// 鼠标弹起事件
void hn2DCameraView::mouseReleaseEvent(QMouseEvent *event)
{
	m_bPushMiddleButton = false;
}

// 鼠标移动事件
void hn2DCameraView::mouseMoveEvent(QMouseEvent *event)
{
...
void hn2DCameraView::keyPressEvent(QKeyEvent *e)
{
...
// 自定义坐标转换屏幕坐标
QPoint hn2DCameraView::fromUser2Screen(const QPoint& pti)
{
	QPoint pt;
...
// 屏幕坐标转换为自定义坐标
QPoint hn2DCameraView::fromScreen2User(const QPoint& pti)
{
	QPoint pt;
	pt.rx() = pti.x();
	pt.ry() = pti.y();
...
void hn2DCameraView::getOldCenterByMoveImage(int X, int Y)
{
	m_pMidClick = QPoint(X, Y) - m_pCenter;
}

//获得新的移动点 鼠标中键移动 用于移动图片
void hn2DCameraView::getNewCenterByMoveImage(int X, int Y)
{
	QPoint pCenter;
	pCenter = QPoint(X, Y);
...
//放大缩小图片
void hn2DCameraView::scaleImage(short zDelta)
{
...
bool hn2DCameraView::addImage(const QString& picPath)
{
	//法3
	if (NULL != m_LoadPic)
	{
...
//ptInImage
bool hn2DCameraView::ptInImage(const QPoint& p)
{
	if (m_LoadPic == NULL)
	{
		return false;
...
// 绘制图片
void hn2DCameraView::drawImage(QPainter * painter)
{
	painter->save();
...
// 绘制添加病害
void hn2DCameraView::drawAddDisease(QPainter* painter)
{
	
}

// 绘制影像病害
void hn2DCameraView::drawImageDisease(QPainter* painter)
{
...
// 设置显示状态
void hn2DCameraView::setShowState(bool bShow)
{
	m_bShow = bShow;
...
// 更新绘制数据
void hn2DCameraView::updateDrawData()
{
