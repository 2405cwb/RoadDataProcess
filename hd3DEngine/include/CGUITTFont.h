...
namespace gui
{
	// wchar_t and char convert each other [zhangfei 2014/4/12]
	class CUserStringConvert
	{
	public:
		CUserStringConvert();
		virtual ~CUserStringConvert();
		// 宽字节转单字节
	...
	class CGUITTFont : public IGUIFont
	{
		public:
			...
			virtual void SetFontSize(EHD_FONT_SIZE nSize);

			virtual u32 GetFontSize(){return (EHD_FONT_SIZE)size;}

			virtual wchar_t* CharToWchar(const char* c);

			virtual bool IsExistWideChar(const char* str);
		...
		private:
			...
			// 字符转换类
			CUserStringConvert m_StringConvert;
	};
