...
#include "QCursor"
#define DEBUG_PRINT_hnRibbonToolButton 1
#if DEBUG_PRINT_hnRibbonToolButton
...
hnRibbonToolButton::hnRibbonToolButton(QWidget *parent)
    :QToolButton(parent)
...
hnRibbonToolButton::hnRibbonToolButton(QAction *defaultAction, QWidget *parent)
    :QToolButton(parent)
    ,m_buttonType(LargeButton)
    ,m_mouseOnSubControl(false)
    ,m_borderColor(242,202,88)
...
void hnRibbonToolButton::paintEvent(QPaintEvent *event)
{
    switch(m_buttonType)
    {
...
void hnRibbonToolButton::resizeEvent(QResizeEvent *e)
{
    QStyleOptionToolButton opt;
    initStyleOption(&opt);
    m_iconRect = QRect(0,0,height(),height());
    QSize iconSize = hnRibbonDrawHelper::iconActualSize(opt.icon,&opt,m_iconRect.size().boundedTo(opt.iconSize));
...
void hnRibbonToolButton::mouseMoveEvent(QMouseEvent *e)
{
...
void hnRibbonToolButton::mousePressEvent(QMouseEvent *e)
{
...
void hnRibbonToolButton::mouseReleaseEvent(QMouseEvent *e)
{
    QToolButton::mouseReleaseEvent(e);
    m_menuButtonPressed = false;
}

void hnRibbonToolButton::focusOutEvent(QFocusEvent *e)
{
    QToolButton::focusOutEvent(e);
    m_mouseOnSubControl = false;
}

void hnRibbonToolButton::leaveEvent(QEvent *e)
{
    m_mouseOnSubControl = false;
...
void hnRibbonToolButton::paintSmallButton(QPaintEvent *e)
{
    Q_UNUSED(e);
...
void hnRibbonToolButton::paintLargeButton(QPaintEvent *e)
{
    Q_UNUSED(e);
...
bool hnRibbonToolButton::hitButton(const QPoint &pos) const
{
...
void hnRibbonToolButton::drawIconAndLabel(QPainter &p, const QStyleOptionToolButton &opt)
{
    if(LargeButton == m_buttonType)
    {
        //绘制图标和文字
...
//        style()->drawControl(QStyle::CE_ToolButtonLabel, &label, &p, this);
    ...
    else
    {
        ...
        if (!opt.icon.isNull())
        {
            pm = hnRibbonDrawHelper::iconToPixmap(opt.icon,this,&opt,opt.rect.size().boundedTo(opt.iconSize));
            pmSize = pm.size() / pm.devicePixelRatio();
...
void hnRibbonToolButton::drawArrow(const QStyle *style, const QStyleOptionToolButton *toolbutton, const QRect &rect, QPainter *painter, const QWidget *widget)
{
...
hnRibbonToolButton::RibbonButtonType hnRibbonToolButton::buttonType() const
{
    return m_buttonType;
}

void hnRibbonToolButton::setButtonType(const RibbonButtonType &buttonType)
{
...
QSize hnRibbonToolButton::minimumSizeHint() const
{
    return sizeHint();
}

bool hnRibbonToolButton::event(QEvent *e)
{
    switch(e->type())
    {
