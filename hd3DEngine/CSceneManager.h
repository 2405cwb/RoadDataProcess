...
namespace scene
{
	...
	*/
	...
	public:
		...
		//! Adds a camerascene node which is able to be controled with th mouse and keys
		//! like in the Software FaroScene
		//! Added by yaoli on 2012/05/21
		virtual ICameraSceneNode* addCameraSceneNodeCus(scene::ESCENE_NODE_CUSANIMATOR_STYLE style = scene::ESNCS_CAMERA,
			ISceneNode* parent = 0,
			f32 rotateSpeed = -1500.0f, f32 zoomSpeed = 200.0f,
			f32 translationSpeed = 500.0f, s32 id=-1,
			bool makeActive=true);
		...
		virtual ISceneNode* addSkyDomeSceneNode(video::ITexture* texture,
			...
			f32 texturePercentage=0.9, f32 spherePercentage=2.0,f32 radius = 100.f,
		...
		//! 得到摄像机的animator，added by yaoli;
		virtual ISceneNodeAnimatorCameraCus* GetCameraSceneNodeAnimator(ESCENE_NODE_ANIMATOR_TYPE type);

		//! 根据屏幕坐标获取视椎体
		virtual void GetViewFrustum(const irr::core::recti& srcRect,SViewFrustum* pViewFrustum);

		virtual bool IsAnimating();

		virtual void SetAnimateState(bool bAnimating);

		virtual gui::IGUIFont* GetBuiltInFont();

		virtual gui::IGUIFont* GetBuiltDefaultFont();

		virtual gui::IGUIFont* GetChineseFont();

		// 根据输入的字符判断是否存在宽字符（中文字符），并返回相应的IGUIFont [zhangfei]
		virtual gui::IGUIFont* GetAdaptiveFont(const char* srcStr, bool& bHaveWideChar);

		// 设置刷新渲染类型
		virtual void SetRefreshType(ESCENE_REFRESH_TYPE type);
		virtual ESCENE_REFRESH_TYPE GetRefreshType( );
	...
	private:
		...
		// 刷新类型
		ESCENE_REFRESH_TYPE m_refreshType;
	};
