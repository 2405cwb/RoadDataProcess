...
public:
	...
	bool writeAndFormat(int row, int col, const QVariant &value,int formatRow);

	//删除列
	void deleteColumn(int colmun);

	//删除的是最后一列
	void deleteLastColumn(int colmun);

	//cwb  复制目标区域到  以起点的地点
	void copyRange(  const QString & sourceRange , const QString targetStart);

	QVariant read(const CellReference &cell) const;
	QVariant read(int row, int col) const;
	QVariant readCellValue(int row, int col) const;
	...
	//交换excel两列的位置,startRow 从1开始
	void swapColumns( int startRow,int col1, int col2);

	void reverseRowsFrom(int sCol, int eCol , int startRow, int endRow = 0);
	...
	QList<QList<QVariant>> readExcelRange(int sRowIndex, int scolIndex, int eColIndex, const QString & filePath, const QString& sheetName);
private:
