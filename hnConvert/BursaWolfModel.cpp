...
int CBursaWolfModel::invers_matrix(double *m1, int n) const
{
	int *is, *js;
	int i, j, k, l, u, v;
	double temp, max_v;
...
int CBursaWolfModel::invers_matrix(float *m1, int n) const
{
	int *is, *js;
	int i, j, k, l, u, v;
	float temp, max_v;
...
// 创建自定义坐标系与84坐标系转换模型
void CBursaWolfModel::createTransModel(double dCenterX, double dCenterY, double dCenterZ, double dNormalX,
	double dNormalY, double dNormalZ)
{
	//绕Z轴旋转角度
	double dYaw = 0.0;
	dYaw = atan2((double)dNormalY, (double)dNormalX) * 57.29577951308233;
	if (dYaw < 0)
	{
	...
	dYaw = dYaw - 90;
	dYaw = dYaw * DEGTORAD64;

	// 绕Y轴旋转角度计算
	double vertiAngle = atan2((double)dNormalZ,
		sqrt((double)dNormalX * (double)dNormalX + (double)dNormalY * (double)dNormalY)) * 57.29577951308233;
	...
	vertiAngle = vertiAngle * DEGTORAD64;

	//计算转换矩阵
	IrrAngle2RotateMatrix(vertiAngle, 0.0, dYaw);

	//设置偏移量
	m_fOffset[0] = dCenterX;
	m_fOffset[1] = dCenterY;
	m_fOffset[2] = dCenterZ;
	Parameter2matrix();
...
//				//求L0
//				pLPos[i].x =  (double)(x*cos(theta) + y*sin(theta) + del_x);
//				pLPos[i].y = (double)(-x*sin(theta) + y*cos(theta) + del_y);
//				pLPos[i].z = 0;
//			}
//
//			//2.求l,l = L - L0
