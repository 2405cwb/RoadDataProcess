...
namespace
{
	...
	QString diseaseToastTitle(DiseaseToastMode mode, int drawType)
	{
		...
		case DiseaseToastMode::Add:
			commandName = QStringLiteral("添加病害 F1");
			break;
		case DiseaseToastMode::Delete:
			commandName = QStringLiteral("删除病害 F2");
			break;
		case DiseaseToastMode::Edit:
			commandName = QStringLiteral("编辑病害 F3");
			break;
		}
	...
	QString diseaseToastContent(DiseaseToastMode mode, int drawType)
	{
		...
		{
		case DiseaseToastMode::Add:
			currentCommand = QStringLiteral("当前操作：添加病害 F1");
			break;
		case DiseaseToastMode::Delete:
			currentCommand = QStringLiteral("当前操作：删除病害 F2");
			break;
		case DiseaseToastMode::Edit:
			currentCommand = QStringLiteral("当前操作：编辑病害 F3");
			break;
		}

		QStringList lines;
		lines << QStringLiteral("当前工程：%1").arg(drawModeName(drawType));
		...
		{
		case 0:
			lines << QStringLiteral("添加：F1 后左键拖框/点选绘制，右键取消当前绘制");
			lines << QStringLiteral("删除：F2 后左键点击病害删除");
			lines << QStringLiteral("编辑：F3 后左键点击病害区域编辑");
			lines << QStringLiteral("普通模式：左键选中病害，Delete 删除当前选中病害");
			break;
		case 1:
			lines << QStringLiteral("添加：F1 后左键开始/完成自动化小框病害");
			lines << QStringLiteral("D：切换拉框填充小方格；拖动时只显示大框，确认后显示小方格病害");
			lines << QStringLiteral("B：切换线状小框绘制；N 结束线状绘制并进入病害选择");
			lines << QStringLiteral("删除：F2 后左键删除整个病害，右键删除单个小方格，按住右键拖动连续删除小方格");
			lines << QStringLiteral("编辑：F3 后左键点击病害区域编辑");
			lines << QStringLiteral("普通模式：左键选中病害，Delete 删除当前选中病害");
			break;
		case 2:
			lines << QStringLiteral("添加：请使用添加面状病害/添加线状病害按钮");
			lines << QStringLiteral("删除：F2 后左键点击设计病害删除");
			lines << QStringLiteral("编辑：F3 后左键点击设计病害编辑");
			lines << QStringLiteral("普通模式：左键选中病害，Delete 删除当前选中病害");
			break;
		default:
			lines << QStringLiteral("当前工程绘制模式未知，请检查工程设置");
			break;
		}
...
void hnRoadDataProcess::allWidgetLoadPictures(BusyLoadingGuard* loading)
{
	...
	if (loading)
	{
		...
		loading->setMessage(QStringLiteral("正在准备视图图片..."));
	}
	...
	if (curProject)
	{
		...
		{
			updateLoading(40, QStringLiteral("正在加载二维路面影像..."));
			stepTimer.start();
			this->m_2dPixScrollWidget->loadRoadPicture();
			qDebug().noquote() << "[HN_PERF][AllWidgetLoadStep]" << "step=2dRoadPicture" << "elapsedMs=" << stepTimer.elapsed();
			updateLoading(55, QStringLiteral("二维路面影像加载完成"));

			if (m_pStreetViewWidget)
			{
				updateLoading(60, QStringLiteral("正在加载景观影像..."));
				stepTimer.restart();
				m_pStreetViewWidget->initView();
				qDebug().noquote() << "[HN_PERF][AllWidgetLoadStep]" << "step=streetView" << "elapsedMs=" << stepTimer.elapsed();
				updateLoading(68, QStringLiteral("景观影像加载完成"));
			}
			if (hnDataManager::getDataManager()->getCurrentProject()->getProjectType() != PROJECT_2D_TYPE)
			{
				updateLoading(72, QStringLiteral("正在加载三维影像..."));
				stepTimer.restart();
				m_3dPixScrollWidget->load3dImage();
				qDebug().noquote() << "[HN_PERF][AllWidgetLoadStep]" << "step=3dImage" << "elapsedMs=" << stepTimer.elapsed();
				updateLoading(78, QStringLiteral("三维影像加载完成"));
			}
		}
		else if (curProject->getProjectType() == PROJECT_TYPE::PROJECT_2D_TYPE)
		{
			updateLoading(45, QStringLiteral("正在加载二维路面影像..."));
			stepTimer.start();
			this->m_2dPixScrollWidget->loadRoadPicture();
			qDebug().noquote() << "[HN_PERF][AllWidgetLoadStep]" << "step=2dRoadPicture" << "elapsedMs=" << stepTimer.elapsed();
			updateLoading(65, QStringLiteral("二维路面影像加载完成"));

			if (m_pStreetViewWidget)
			{
				updateLoading(72, QStringLiteral("正在加载景观影像..."));
				stepTimer.restart();
				m_pStreetViewWidget->initView();
				qDebug().noquote() << "[HN_PERF][AllWidgetLoadStep]" << "step=streetView" << "elapsedMs=" << stepTimer.elapsed();
				updateLoading(78, QStringLiteral("景观影像加载完成"));
			}
		}
		else
		{
			if (hnDataManager::getDataManager()->getCurrentProject()->getProjectType() != PROJECT_2D_TYPE)
			{
				updateLoading(55, QStringLiteral("正在加载三维影像..."));
				stepTimer.start();
				m_3dPixScrollWidget->load3dImage();
				qDebug().noquote() << "[HN_PERF][AllWidgetLoadStep]" << "step=3dImage" << "elapsedMs=" << stepTimer.elapsed();
				updateLoading(78, QStringLiteral("三维影像加载完成"));
...
void hnRoadDataProcess::openProjectSlot()
{
	...
	loading.setMessage(QStringLiteral("正在加载所有视图的图片数据..."));
		...
		loading.setMessage(QStringLiteral("正在更新所有界面..."));
...
void hnRoadDataProcess::slot_outAllResultDatas()
{
	...
	int currentTask = 0;
	//创建进度对话框
	QProgressDialog progress(QStringLiteral("处理工程数据..."), QStringLiteral("取消"), 0, totalTasks * progressScale);
	progress.setWindowModality(Qt::WindowModal);  //模态
	progress.setMinimumDuration(0);   //立即显示
	for each (auto project in allPorject)
	{
		...
		for (int splitValue : splitValueVec)
		{
			...
			progress.setLabelText(QStringLiteral("正在处理 %1 (%2m)...").arg(project->get2DProName()).arg(splitValue));
