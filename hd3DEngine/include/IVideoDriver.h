	...
	*/
	...
	public:
		...
		*/
		...
		//! 设置背景颜色
		virtual void setGradientBackground(SColor color=SColor(255,0,0,0)) =0;
		...
		virtual void draw3DPointList(const void* vertices,u32 vertexCount,void* colorBuf,E_VERTEX_TYPE type) = 0;

		//! 绘制三维点-龚书林
		virtual void draw3DPoint(const core::vector3df& pt,SColor color = SColor(255,255,255,255)) = 0;
		//! 绘制三维点-龚书林
		virtual void draw3DPoint(float x,float y,float z,SColor color = SColor(255,255,255,255)) = 0;

		//! 设置绘制三维状态-龚书林
		virtual void setRenderStates3DMode() = 0;

		//! 绘制不规则二维多边形
		virtual void draw2DPoly(bool &drawSuccess, const core::position2d<f32>* points2d, u32 vertexCount,SColor color=SColor(255,255,255,255)) = 0;

		//! 绘制三角形
		virtual void draw2DTriangle(const core::position2d<f32>* points2d, u32 vertexCount,SColor color=SColor(255,255,255,255)) = 0;
		//! 绘制球体
		virtual void drawSphere(const core::vector3d<f32> center3d, f32 radius,SColor color=SColor(255,255,255,255)) = 0;

		//! 绘制三维空间圆形
		virtual void drawCircle(const core::vector3d<f32> center3d, f32 radius, const core::vector3d<f32> normal3d, u32 count, SColor color=SColor(255,255,255,255)) = 0;

		//! 绘制二维圆形
		virtual void draw2DCircle(const core::position2di src, int radius, SColor color = SColor(255,255,255,255)) = 0;
		
        //! 绘制二维椭圆
		virtual void draw2DEllipse(const core::position2di src,int rx,int ry,SColor color = SColor(255,255,255,255)) = 0;

		//! 用虚线绘制二维线段
		virtual void draw2dDashedLine(const core::position2d<s32>& start, const core::position2d<s32>& end, SColor color=SColor(255,255,255,255)) =0;

        //! 用虚线绘制二维线段(针对AMD显卡)
        virtual void draw2dDashedLineAMD(const core::position2d<s32>& start, const core::position2d<s32>& end, SColor color=SColor(255,255,255,255)) =0;

		//! 用虚线绘制三维线段
		virtual void draw3dDashedLine(const core::vector3df& start,
			const core::vector3df& end, SColor color = SColor(255,255,255,255)) = 0;

		//! 绘制三维圆柱体
		virtual void draw3DCylinder(const core::vector3d<f32> start,const core::vector3d<f32> end,f32 radius, SColor color = SColor(255,255,255,255)) =0;

		//绘制三维圆 [2013/09/23 危迟]
		virtual void draw3DCircle(const core::vector3d<f32> center3d, f32 radius, const core::vector3d<f32> normal3d, u32 count, SColor color = SColor(255,255,255,255)) = 0;

		// 绘制三维圆弧(圆弧中心点，起始圆弧射线点，终止圆弧的射线点（射线点不需要在圆上）)-2014.04.11.zhubo
		virtual void draw3DArc(const core::vector3d<f32> center3d,const core::vector3d<f32> arcStart , const core::vector3d<f32> arcEnd , f32 radius, f32 fAngle,u32 count ,SColor color = SColor(255,255,255,255)) = 0;

        // 绘制二维圆弧(圆弧中心点, 起始圆弧射线点，终止圆弧的射线点（射线点不需要在圆上）)-2016.04.07.张阳
        virtual void draw2DArc(const core::vector2di center2d, f32 fBeginAngle, f32 fEndAngle, f32 radius, u32 count, SColor color = SColor(255,255,255,255)) = 0;

		// 绘制直角三角形垂线标记-2014.04.11 add by zhubo
		virtual void draw3DVerticalLine(const core::vector3d<f32> mid3d,const core::vector3d<f32> start3d,const core::vector3d<f32> end3d,f32 radius,SColor color = SColor(255,255,255,255)) = 0;
		...
		*/
		...
		// add by chy 2013/8/17 draw axis
		virtual void DisplayAxis(core::vector3df center,   // data center
			...
			bool	bSolid = true) = 0;                    //  Display as Solid

		virtual void DisplaySingleAxis(core::vector3df start,  // 起点坐标
			...
			int		iSlices,				               //  Number of slices around dia
			int		iStacks,				               //  Number of Stacks about length
			SColor color = SColor(255,255,255,255),		// Axis Color (Use Defaults..)
			bool	bSolid = true) = 0;                    //  Display as Solid

		// add by chy 2013/11/15 draw compass
		virtual void DrawCompass(core::vector3df center,   // data center
			double	dAxisLength,						   // Axis length
			double	dAxisRadius,					 	   // Axis diameter
			double	dArrowLength,						   // Arrow length
			double	dArrowRadius,			               //  Arrow diameter
			...
			float	fColorZ[4],                            //  Z-Axis Color
			bool	bSolid = true) = 0;                    //  Display as Solid
		// add by chy 2014/3/17  draw float line
		virtual void drawFloatLine(core::vector3df center,   // data center
			double	dAxisLength,						   // Axis length
			double	dAxisRadius,					 	   // Axis diameter
			double	dArrowLength,						   // Arrow length
			double	dArrowRadius,			               //  Arrow diameter
