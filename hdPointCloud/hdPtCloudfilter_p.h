...
#ifndef INCLUDE_SPACE_FILTER_HPP
#define INCLUDE_SPACE_FILTER_HPP
...
/*! @class hdPtCloudfilter_p hdPtCloudfilter_p.h
* @ingroup 
* @brief   点云快速去重简化类
* @details 实现点云按空间距离快速去重简化
* @code
	space_filter simpleFilter;
	simpleFilter.set_ptcloud_count(allcount);
	....
	int filterd = simpleFilter.simple_points_f<hdPointXYZIPRGBA>(pt_list._Myfirst,count);
	for (int i = 0;i < filterd;i++)
	{
		hdPointXYZIPRGBA& pt = *(pt_list._Myfirst + i);
	}
* @endcode
*/
class space_filter
...
public:
   ...
   : salt_count_(0),
	 ...
	 simple_dist_(1/0.02f)
   {}

   space_filter(const space_parameters& p)
   ...
   space_filter(const space_filter& filter)
   {
      this->operator=(filter);
   }

   inline bool operator == (const space_filter& f) const
   {
      if (this != &f)
      {
         return
   ...
   inline space_filter& operator = (const space_filter& f)
   {
      if (this != &f)
      {
         salt_count_ = f.salt_count_;
   ...
   //! 设置抽稀空间距离
   void set_simple_space(float dist)
   {
	   simple_dist_ = 1/dist;
   }
   //! 设置总点数
   void set_ptcloud_count(unsigned long long count)
   {
	   unsigned int random_seed = 0xA57EC3B2;

	   const double desired_probability_of_false_positive = 1.0 / count;

	   space_parameters parameters;
	   ...
	   projected_element_count_ = count;
	   inserted_element_count_ = 0;
	   random_seed_ = ((0xA57EC3B2 * 0xA5A5A5A5) + 1),
	   desired_false_positive_probability_ = 1.0 / count;

	   salt_count_ = parameters.optimal_parameters.number_of_hashes;
	   table_size_ = parameters.optimal_parameters.table_size;
   ...
   /*!
	* @brief 抽稀一个unsigned int/unsigned short坐标数组,T包含x,y,z成员
	* @details 抽稀一个unsigned int/unsigned short坐标数组,T包含x,y,z成员
	* @param[in/out] pPoints 输入点数组,抽稀后也保存在该数组
	* @param[in] count 输入点数组个数
	* @param[in] scale 抽稀系数>=1,如果输入坐标精度是0.01,抽稀系数为2,则抽稀距离是0.02
	* @result 返回抽稀后点数
	*/
   template<typename T>
   int simple_points_i( T* pPoints, const int& count, const unsigned int& scale /*= 1*/ )
   {
	   int filterIndex = 0;
	   for (int i = 0;i < count;i++)
	   {
		   const T& pt = *(pPoints + i);
		   tmpPt_.x = pt.x / scale;
		   tmpPt_.y = pt.y / scale;
		   tmpPt_.z = pt.z / scale;
		   if (!contains(reinterpret_cast<const unsigned char*>(&tmpPt_),sizeof(pointxyz_i)))
		   {
			   insert(reinterpret_cast<const unsigned char*>(&tmpPt_),sizeof(pointxyz_i));
			   // 不重复点,向前移动
			   memcpy(pPoints + filterIndex,pPoints + i,sizeof(T));
			   filterIndex++;
		   }
	   }
   ...
   /*!
	* @brief 抽稀一个float坐标数组,T包含x,y,z成员
	* @details 抽稀一个float坐标数组,T包含x,y,z成员
	* @param[in/out] pPoints 输入点数组,抽稀后也保存在该数组
	* @param[in] count 输入点数组个数
	* @result 返回抽稀后点数
	*/
   template<typename T>
   int simple_points_f( T* pPoints, const int& count )
   {	   
	   int filterIndex = 0;
	   for (int i = 0;i < count;i++)
	   {
		   const T& pt = *(pPoints + i);
		   tmpPt_.x = pt.x * simple_dist_;
		   tmpPt_.y = pt.y * simple_dist_;
		   tmpPt_.z = pt.z * simple_dist_;
		   if (!contains(reinterpret_cast<const unsigned char*>(&tmpPt_),sizeof(pointxyz_i)))
		   {
			   insert(reinterpret_cast<const unsigned char*>(&tmpPt_),sizeof(pointxyz_i));
			   // 不重复点,向前移动
...
protected:
   ...
   //! 点云抽稀临时点
   pointxyz_i				  tmpPt_;
   //! 过滤距离,默认值1/0.02,即0.02m抽稀
   float					  simple_dist_;
};

inline space_filter operator & (const space_filter& a, const space_filter& b)
{
   space_filter result = a;
   result &= b;
   return result;
}
...
class compressible_space_filter : public space_filter
{
public:

   compressible_space_filter(const space_parameters& p)
   : space_filter(p)
   {
