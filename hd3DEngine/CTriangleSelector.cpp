...
CTriangleSelector::CTriangleSelector( const IMeshBuffer* meshBuf, const ISceneNode* node )
	: SceneNode(node), AnimatedNode(0), LastMeshFrame(0)
{
#ifdef _DEBUG
	setDebugName("CTriangleSelector");
	...
	if (meshBuf)
	{
		u32 faceCount = meshBuf->getIndexCount();
		faceCount /= 3;
		Triangles.reallocate(faceCount);
		createFromMesh(meshBuf);
	}
}
...
void CTriangleSelector::createFromMesh(const IMeshBuffer* meshBuf)
{
	//createFromMesh(meshBuf);
	const u32 idxCnt = meshBuf->getIndexCount();
	const u32* const indices = meshBuf->getIndices();

	for (u32 j=0; j<idxCnt; j+=3)
	{
		Triangles.push_back(core::triangle3df(
				meshBuf->getPosition(indices[j+0]),
				meshBuf->getPosition(indices[j+1]),
				meshBuf->getPosition(indices[j+2])));
	}
}
...
void CTriangleSelector::updateFromMesh(const IMesh* mesh) const
{
	...
	for (u32 i = 0; i < meshBuffers; ++i)
	{
		...
		const u32* indices = buf->getIndices();
