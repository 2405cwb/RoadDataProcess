...
class line3d
{
	public:
		...
		*/
		...
		//! 求点到线距离
		T getDistanceToPoint(const vector3d<T>& point) const
		{
			vector3d<T> pt = getClosestPoint(point);
			return pt.getDistanceFrom(point);
		}

		//! 求点到线的距离 [危迟]
		//! 将点绕直线旋转90度 将旋转后的点与直线构建平面 求解点到平面的距离即为点到直线的距离
		T getDistanceToPointP(const vector3d<T>& point) const
		{
			vector3d<T> dir = getVector();
			vector3d<T> rotPt(point);

			rotPt.RotateByVector(90.f,start,dir);

			plane3d<T> plane(start,end,rotPt);

		    return plane.getDistanceTo(point);
		}

		//! 求三维点到直线的垂足点 [2013/10/14 危迟]
		// 根据直线与点构建平面，求解平面与直线的交点，即为垂足点
		bool getFOPFromPointP(const vector3d<T>& point,vector3d<T>& outIntersection) const
		{
			// 法向量
			vector3d<T> dir = getVector();
			dir.normalize();
			...
			// 构建平面
			plane3d<T> plane(point,dir);

			// 计算平面与直线
			plane.getIntersectionWithLine(start,getVector(),outIntersection);
