...
void HFixCalus2(int Model, ZHDHFixPar par, double x, double y, double *dResult)
{
	*dResult = 0;
	...
	case ZHD_HFixEnum_Pane:
		...
		*dResult = par.A + par.B * (x - par.X0) + par.C * (y - par.Y0);
		break;
	case ZHD_HFixEnum_Curve:
		//*dResult = par.A + par.B * x + par.C * y + par.D * pow(x, 2) + par.E * pow(y, 2)+ par.F* x * y  ;
		double dtmpx = x - par.X0;
		double dtmpy = y - par.Y0;
		*dResult = par.A + par.B * dtmpx + par.C * dtmpy + par.D * pow(dtmpx, 2) + par.E * pow(dtmpy, 2) + par.F* dtmpx * dtmpy;
		break;
	}
}
