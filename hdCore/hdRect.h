...
namespace hd
{
	...
	*/
	...
	public:
		...
		CHdRect() : UpperLeftCorner(0,0), LowerRightCorner(0,0) {}

		//! Constructor with two corners
		CHdRect(T x, T y, T x2, T y2)
			: UpperLeftCorner(x,y), LowerRightCorner(x2,y2) {}

		//! Constructor with two corners
		CHdRect(const CHdPosition2d<T>& upperLeft, const CHdPosition2d<T>& lowerRight)
		...
		CHdRect<T> operator+(const CHdPosition2d<T>& pos) const
		{
			CHdRect<T> ret(*this);
			return ret+=pos;
		}
		...
		//! move right by given numbers
		CHdRect<T>& operator+=(const CHdPosition2d<T>& pos)
		{
			UpperLeftCorner += pos;
		...
		//! equality operator
		bool operator==(const CHdRect<T>& other) const
		{
		...
		bool isPointInside(const CHdPosition2d<T>& pos) const
		{
		...
		bool isRectCollided(const CHdRect<T>& other) const
		{
		...
		void clipAgainst(const CHdRect<T>& other)
		{
		...
		bool constrainTo(const CHdRect<T>& other)
		{
		...
		CHdPosition2d<T> getCenter() const
		{
			return CHdPosition2d<T>(
		...
		void addInternalPoint(const CHdPosition2d<T>& p)
		{
		...
		//! Upper left corner
		CHdPosition2d<T> UpperLeftCorner;
		//! Lower right corner
		CHdPosition2d<T> LowerRightCorner;
	};

	//! Rectangle with float values
	typedef CHdRect<float> CHdRectf;
	//! Rectangle with double values
	typedef CHdRect<double> CHdRectd;
	//! Rectangle with int values
	typedef CHdRect<int> recti;
