...
namespace irr
{
	...
	{
		//////////////////////////////////////////////////////////////////////////
		// Tessellation 回调 用于画凹多边形
		// wkl
		//标记多边形是否有效， 自相交的多边形是无效的
		bool bPolyValid;
		...
		void COpenGLDriver::draw3DPointList(const void* vertices,u32 vertexCount,void* colorBuf,E_VERTEX_TYPE type)
		{
			...
			if(colorBuf)
			{
				...
				glColorPointer(4, GL_UNSIGNED_BYTE, sizeof(GLubyte)*4, colorBuf);
			}
			//glVertexPointer(3,GL_FLOAT,0,&(reinterpret_cast<const float*>(vertices))[0]);
		...
		float* clr = (float*)colorBuf;
		float* vec = (float*)vertices;
		for (u32 i = 0;i<vertexCount;i++)
		{
		glVertex3fv(&(vec[i*3]));
		}
		...
		*/
		//! 绘制三维点-龚书林
		void COpenGLDriver::draw3DPoint(const core::vector3df& pt,SColor color)
		{
			draw3DPoint(pt.X,pt.Y,pt.Z,color);
			/*setRenderStates3DMode();
		...
		//! 绘制三维点-龚书林
		void COpenGLDriver::draw3DPoint(float x,float y,float z,SColor color)
		{
			setRenderStates3DMode();
		...
		//! 绘制凹/凸多边形-汪开理
		// drawSuccess 返回值：多边形是否绘制成功
		// wkl
		void COpenGLDriver::draw2DPoly( bool &drawSuccess, const core::position2d<f32>* points2d, u32 vertexCount,SColor color/*=SColor(255,255,255,255)*/ )
		{
			bPolyValid = true;
			disableTextures();
			...
			GLUtesselator *tess = gluNewTess();// create a tesselator
			if(!tess) return;
			...
			GLdouble **poly = new GLdouble *[vertexCount];

			for (unsigned int i = 0;i<vertexCount;i++)
			{
				poly[i] = new GLdouble[3];
			}
			for (unsigned int i = 0;i<vertexCount;i++)
			{
				poly[i][0] = points2d[i].X;
				poly[i][1] = points2d[i].Y;
				poly[i][2] = 0;
			}
			...
			delete[]poly;
			poly = NULL;
			drawSuccess = bPolyValid;
		}

		//! 绘制二维三角形-危迟
		void COpenGLDriver::draw2DTriangle(const core::position2d<f32>* points2d, u32 vertexCount,SColor color/* =SColor */)
		{
			if (vertexCount != 3)
			{
				return;
			...
			glVertex2f(points2d[0].X, points2d[0].Y);
			glVertex2f(points2d[1].X, points2d[1].Y);
			glVertex2f(points2d[2].X, points2d[2].Y);
		...
		//! 绘制球体-汪开理
		// wkl
		void COpenGLDriver::drawSphere(const core::vector3d<f32> center3d, f32 radius,SColor color/*=SColor(255,255,255,255)*/)
		{
			disableTextures();
			setRenderStates3DMode();
			...
			//static const GLfloat mat_shininess = 30.0f;

			//glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, blue_color);
		...
		void COpenGLDriver::draw2DCircle(const core::position2di src, int radius, SColor color)
		{
			disableTextures();
			setRenderStates2DMode(color.getAlpha() < 255, false, false);
			...
			glColor4ub(color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha());
			float i = 0.0;
			for (i = 0.0; i <= 2 * pi; i += pi/180)
			{
				glVertex2f(src.X + cos(i) * radius, src.Y + sin(i) * radius);
			}
			glEnd();
		}

		void COpenGLDriver::draw2DEllipse( const core::position2di src,int rx,int ry,SColor color /*= SColor(255,255,255,255)*/ )
		{
			disableTextures();
			setRenderStates2DMode(color.getAlpha() < 255, false, false);
			...
			for (int i = 0;i < 90;i++)
			{
				float degInRAD = i * 4 * irr::core::DEGTORAD;
				glVertex2f(src.X + cos(degInRAD) * rx,src.Y	+ sin(degInRAD) * ry);
			}
			glEnd();
		}
		...
		//! 绘制三维空间圆形-汪开理
		// wkl
		void COpenGLDriver::drawCircle(const core::vector3d<f32> center3d, f32 radius, const core::vector3d<f32> normal3d, u32 count, SColor color/*=SColor(255,255,255,255)*/)
		{
			//先实现在xy平面内画圆
			//关于 根据圆的法向量进行旋转还未实现
			//wkl 2012-7-14 14:54:04
			/*
			disableTextures();
			setRenderStates3DMode();
			...
			core::position2d<f32> first;
			core::position2d<f32> a,b;
			core::position2d<f32> center;
			center.X = center3d.X;
			center.Y = center3d.Y;
			//先在xy平面内画圆
			glMatrixMode(GL_MODELVIEW);
			glPushMatrix();
			core::vector3df normXY = core::vector3df(0,0,1);      

			glRotatef(normal3d.X, normal3d.Y, normal3d.Z);	

			for (s32 j=0; j<count; ++j)
			{		
			f32 p = (f32)j / count * (core::PI*2);
			a = center + core::position2d<f32>((f32)(sin(p)*radius), (f32)(cos(p)*radius));
			if (j==0)
			first = a;
			else
			draw3DLine(core::vector3df(a.X,a.Y,center3d.Z),core::vector3df(b.X,b.Y,center3d.Z),color);
			b = a;

			}
			draw3DLine(core::vector3df(a.X,a.Y,center3d.Z),core::vector3df(first.X,first.Y,center3d.Z),color);
		...
		// 绘制三维圆
		void COpenGLDriver::draw3DCircle(const core::vector3d<f32> center3d, f32 radius, const core::vector3d<f32> normal3d, u32 count, SColor color /* = SColor */)
		{
			disableTextures();
			setRenderStates3DMode();

			core::vector3df tmpCenter(center3d.X,center3d.Y,center3d.Z);
			// 获取圆所在平面上一点
			core::vector3df samt;
			tmpCenter.GetPlanarPoint(normal3d,samt);

			samt = samt - tmpCenter;
			
			core::vector3df sample = tmpCenter + samt.normalize()*radius;

			f32 sliceAngle = 360.f/(f32)count;
			
			irr::u32 r = color.getRed();
			irr::u32 g = color.getGreen();
			irr::u32 b = color.getBlue();
			irr::u32 a = color.getAlpha();
			//glColor4ub(r,g,b,a/*color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha()*/);
			
			for (int i = 1; i<= 720;i++)
			{
				core::vector3df tmpA = sample;
				core::vector3df tmpB = sample;
				// 将该点绕旋转中心旋转 得到圆弧上一点
				tmpA.RotateByVector(i*sliceAngle,tmpCenter,normal3d);
				tmpB.RotateByVector((i + 1)*sliceAngle,tmpCenter,normal3d);
				glBegin(GL_LINES);
				/*glClearColor(0.0, 0.0, 0.0,0.0);
				glClear(GL_COLOR_BUFFER_BIT);*/
				glColor4ub(r,g,b,a/*color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha()*/);
				glVertex3f(tmpA.X, tmpA.Y, tmpA.Z);
				glVertex3f(tmpB.X, tmpB.Y, tmpB.Z);
				glEnd();
			}
		}

		//! 画三维空间的圆柱体， 指定中起始两点三维坐标，以及圆柱半径
		void COpenGLDriver::draw3DCylinder(const core::vector3d<f32> start,const core::vector3d<f32> end,f32 radius, SColor color /* = SColor */)
		{
			disableTextures();
			setRenderStates3DMode();
			//绘制出圆柱后，圆柱默认是画在Z轴上，需要将圆柱经过旋转和平移操作至指定位置
			//平移和旋转的方法如下：
			//首先将坐标系中心平移至center
			//然后求出两条直线（平移后Z轴和目标轴线）的法向量以及夹角，以及取其法向量上任意一点
			//之后根据夹角和法向量进行旋转
			//绘制圆柱
			...
			//坐标转换 默认是以原点为起点 
			glMatrixMode(GL_MODELVIEW);
			core::vector3d<f32> center;
			center.X = (start.X + end.X);
			center.Y = (start.Y + end.Y);
			center.Z = (start.Z + end.Z);
			core::vector3d<f32> newStart;
			newStart.X = start.X - center.X;
			newStart.Y = start.Y - center.Y;
			newStart.Z = start.Z - center.Z;	
			//计算两个向量之间的夹角以及法向量
			core::vector3d<f32> normal = newStart.crossProduct(core::vector3d<f32>(0,0,1));
			f32 inner = newStart.dotProduct(core::vector3d<f32>(0,0,1));
			f32 degree = acos(inner/(newStart.getLength()*1.0));
			glTranslatef(center.X, center.Y, center.Z);
			glRotatef(degree,normal.X,normal.Y,normal.Z);
			glPushMatrix();
			GLdouble height = (end - start).getLength();
			gluCylinder(cylinderobj,radius,radius,height,100,50);
			glPopMatrix();
			glFlush();
			gluDeleteQuadric(cylinderobj);
		}
		// 蔡红云 绘制三维圆柱坐标轴
		void COpenGLDriver::DisplayAxis(core::vector3df center,   // data center
			double	dAxisLength,						   // Axis length
			double	dAxisRadius,					 	   // Axis diameter
			double	dArrowLength,						   // Arrow length
			double	dArrowRadius,			               //  Arrow diameter
			int		iSlices,				               //  Number of slices around dia
			int		iStacks,				               //  Number of Stacks about length
			float	fColorX[4],				               //  X-Axis Color (Use Defaults..)
			float	fColorY[4],                            //  Y-Axis Color
			float	fColorZ[4],                            //  Z-Axis Color
			bool	bSolid)                                //  Display as Solid

		{

			GLfloat		fXAxisColor[4];
			GLfloat		fYAxisColor[4];
			GLfloat		fZAxisColor[4];

			// Assign Default values to member attributes
			fXAxisColor[0] = 1.00f; // Red
			fXAxisColor[1] = 0.00f;
			fXAxisColor[2] = 0.00f;
			fXAxisColor[3] = 1.0f;

			fYAxisColor[0] = 0.00f; // Green
			fYAxisColor[1] = 1.00f;
			fYAxisColor[2] = 0.00f;
			fYAxisColor[3] = 1.0f;

			fZAxisColor[0] = 0.00f; // Blue
			fZAxisColor[1] = 0.00f;
			fZAxisColor[2] = 1.00f;
			fZAxisColor[3] = 1.0f;
			...
			if(fColorZ)
				...
				glColor4fv(fZAxisColor);
			glMatrixMode(GL_MODELVIEW);
			glPushMatrix();
			glTranslated(center.X, center.Y, center.Z);

			//glTranslated(0, 0, dArrowPosn);
			gluCylinder(m_pQuadric, dAxisRadius, dAxisRadius, dAxisLength, iSlices, iStacks);
			glTranslated(0,0, dArrowPosn);	// Move to arrow position
			...
			if(fColorX)
				...
				glColor4fv(fXAxisColor);

			glTranslated(0, 0, -dArrowPosn);// Move to 0, 0, 0
		...
		void COpenGLDriver::DisplaySingleAxis(core::vector3df start,  // 起点坐标
			core::vector3df end,							// 终点坐标
			double	dAxisRadius,					 	   // Axis diameter
			double	dArrowLength,						        // Arrow length
			double	dArrowRadius,			                    //  Arrow diameter
			int		iSlices,				                    //  Number of slices around dia
			int		iStacks,				                    //  Number of Stacks about length
			SColor  color,				                    //  Axis Color (Use Defaults..)
			bool	bSolid )                                    //  Display as Solid
		{

			GLfloat		fAxisColor[4];

			// Assign Default values to member attributes
			fAxisColor[0] = (GLfloat)color.getRed() / 255.f; // Red
			fAxisColor[1] = (GLfloat)color.getGreen()/ 255.f;
			fAxisColor[2] = (GLfloat)color.getBlue()/ 255.f;
			fAxisColor[3] = 1.0f;

			double dAxisLength = (end - start).getLength();
		...
		// 蔡红云 绘制三维圆柱指北针
		void COpenGLDriver::DrawCompass(core::vector3df center,   // data center
			double	dAxisLength,						   // Axis length
			double	dAxisRadius,					 	   // Axis diameter
			double	dArrowLength,						   // Arrow length
			double	dArrowRadius,			               //  Arrow diameter
...

		{
			...
			// 画完箭头，回到原来状态
			glTranslated(0,0, -dArrowPosn);
			// 在平移dAxisLength距离画一个圆柱
			glTranslated(0,0, -dAxisLength);

			gluCylinder(m_pQuadric, dAxisRadius, dAxisRadius, dAxisLength, iSlices, iStacks);
			// 在画一个箭头
			//glRotated(180, 1.0, 0.0, 0.0);	// Rotate for X
			//
			//gluCylinder(m_pQuadric, dArrowRadius, 0.0f, dArrowLength, iSlices, iStacks);
			...
			// 重置状态
			//glRotated(-180, 1.0, 0.0, 0.0);	// Rotate for X
			glTranslated(0, 0, dAxisLength);// Move to 0, 0, 0
		...
		// 画ArcGIS测图浮动线
		void COpenGLDriver::drawFloatLine(core::vector3df center,   // data center
			double	dAxisLength,						   // Axis length
			double	dAxisRadius,					 	   // Axis diameter
			double	dArrowLength,						   // Arrow length
			double	dArrowRadius,			               //  Arrow diameter
			int		iSlices,				               //  Number of slices around dia
			int		iStacks,				               //  Number of Stacks about length
			float	fColor[4],				               //  Color (Use Defaults..)
			bool	bSolid)                                //  Display as Solid

		{
			...
			fZAxisColor[2] = 1.00f;
			fZAxisColor[3] = 1.00f;
			GLdouble dArrowPosn = dAxisLength;
			disableTextures();
			setRenderStates3DMode();
			// 控制平滑显示还是光滑显示
			//glShadeModel(GL_FLAT);
			...
			glPushMatrix();
			glTranslated(center.X, center.Y, center.Z);
			glRotated(-180, 0.0, 1.0, 0.0);	// Rotate for Y
			
			gluCylinder(m_pQuadric, dArrowRadius, 0.0f, dArrowLength, iSlices, iStacks);
		   
			glRotated(180, 0.0, 1.0, 0.0);	// Rotate for Y
			gluCylinder(m_pQuadric, dAxisRadius, dAxisRadius, dAxisLength, iSlices, iStacks);
			glTranslated(0,0, dArrowPosn + 0.5);	// Move to arrow position
			
			// 画球
			gluSphere(m_pQuadric,0.5, 36, 36);
		...
		//! 用虚线绘制二维线段
		void COpenGLDriver::draw2dDashedLine(const core::position2d<s32>& start, 
			const core::position2d<s32>& end, SColor color)
		{
			...
			glLineStipple(1,0x0F0F);//第一个参数是倍数，第二个是16进制数   
        ...
        //! 绘制二维虚线段(draw2dDashedLine接口绘制虚线时无法再AMD显卡下显示) 默认隔4个像素绘制虚线
        void COpenGLDriver::draw2dDashedLineAMD(const core::position2d<s32>& start, 
            const core::position2d<s32>& end, SColor color/*=SColor(255,255,255,255)*/)
        {
            double dis = (end.Y - start.Y) * (end.Y - start.Y) + (end.X - start.X) * (end.X - start.X);
            dis = sqrt(dis);
            long count = dis / 4; // 隔4个像素
            if (dis - count * 4.0 > 0)
            {
                ++count;
            }
            ...
            {
                aftX = (s32)(1.0 * (i + 1) / count * end.X + 1.0 * (count - i - 1) / count * start.X);
                aftY = (s32)(1.0 * (i + 1) / count * end.Y + 1.0 * (count - i - 1) / count * start.Y);
                glVertex2f(GLfloat(preX), GLfloat(preY));
                glVertex2f(GLfloat(aftX), GLfloat(aftY));

                preX = (s32)(1.0 * (i + 2) / count * end.X + 1.0 * (count - i - 2) / count * start.X);
                preY = (s32)(1.0 * (i + 2) / count * end.Y + 1.0 * (count - i - 2) / count * start.Y);
            }
		...
		//! 用虚线绘制三维线段
		void COpenGLDriver::draw3dDashedLine(const core::vector3df& start,
		...
		void COpenGLDriver::setRenderStates2DMode(bool alpha, bool texture, bool alphaChannel)
		{
			if (CurrentRenderMode != ERM_2D || Transformation3DChanged)
			{
				...
				if (!OverrideMaterial2DEnabled)
				{
					...
					InitMaterial2D.Thickness = Material.Thickness;
		...
		void COpenGLDriver::setGradientBackground(SColor color)
		{
			//clearBuffers(true, false, false, color);
			//glClear(GL_COLOR_BUFFER_BIT);
			//glClear(GL_COLOR_BUFFER_BIT);
			//glBegin(GL_QUADS);
			...
			//glVertex2f(ViewPort.UpperLeftCorner.X,ViewPort.UpperLeftCorner.Y);

			//glColor3ub(color.getRed(), color.getGreen(), color.getBlue());
			//glVertex2f(ViewPort.LowerRightCorner.X, ViewPort.UpperLeftCorner.Y);

			//glColor3ub(0, 0, 0);
			//glVertex2f(ViewPort.UpperLeftCorner.X, ViewPort.LowerRightCorner.Y);

			//glColor3ub(0, 0, 0);
			//glVertex2f(ViewPort.LowerRightCorner.X,ViewPort.LowerRightCorner.Y);
			//glEnd();
			...
			//glColor3f((f32)color.getRed()/255.0, (f32)color.getGreen()/255.0, (f32)color.getBlue()/255.0);
		...
		// 绘制三维圆弧(圆弧中心点，起始圆弧射线点，终止圆弧的射线点（射线点不需要在圆上）)-2014.04.11.zhubo
		void COpenGLDriver::draw3DArc( const core::vector3d<f32> center3d, 
			const core::vector3d<f32> arcStart , 
			const core::vector3d<f32> arcEnd ,
			f32 radius,
			f32 fAngle,
			u32 count ,
			SColor color)
		{
			...
			core::vector3df tmpCenter(center3d.X,center3d.Y,center3d.Z);
			core::vector3df samt(arcStart.X,arcStart.Y,arcStart.Z);
			core::vector3df endt(arcEnd.X,arcEnd.Y,arcEnd.Z);

			// 由起始射线计算法向量,得到该射线与圆弧的交点
			samt = samt - tmpCenter;
			samt = samt.normalize();
			endt = endt - tmpCenter;
			endt = endt.normalize();
			...
			// 计算得到圆弧起始点和终止点
			core::vector3df sample = tmpCenter + samt*radius;
			core::vector3df endSample = tmpCenter + endt* radius;

			// 由起始向量与终止向量计算圆弧所在面的法向量
			core::vector3d<f32> normal3d;
			normal3d.getCrossProduct(samt,endt);
			normal3d.normalize();

			// 由角度值计算每次线段需要旋转的角度
			f32 sliceAngle = fAngle/(f32)count;
			...
			for (int i = 1; i< count;i++)
			{
				...
				core::vector3df tmpB = sample;
				// 将该点绕旋转中心旋转 得到圆弧上一点
				tmpA.RotateByVector((i-1)*sliceAngle,tmpCenter,normal3d);
				tmpB.RotateByVector(i*sliceAngle,tmpCenter,normal3d);
				glBegin(GL_LINES);
				glColor4ub(r,g,b,a);
				glVertex3f(tmpA.X, tmpA.Y, tmpA.Z);
				glVertex3f(tmpB.X, tmpB.Y, tmpB.Z);
        ...
        // 绘制二维圆弧(圆弧中心点, 起始圆弧射线点，终止圆弧的射线点（射线点不需要在圆上）)-2016.04.07.张阳
        void COpenGLDriver::draw2DArc(const core::vector2di center2d,  // 圆弧所在圆的圆心
            f32 fBeginAngle,                                        // 圆弧初始角度
            f32 fEndAngle,                                          // 圆弧结束角度
            f32 radius,                                             // 圆弧半径
            u32 count,                                              // 圆弧实际是由多条线段构成，需要设置线段条数，使其更近似圆弧
            SColor color)                                           // 圆弧线段的颜色值
        {
            disableTextures();
            setRenderStates2DMode(color.getAlpha() < 255, false, false);

            // 分割角度
            f32 tAngle = (fEndAngle - fBeginAngle) / count;

            for (int i = 0; i < count;i++)
            {
                core::vector2di start(center2d.X + cos(fBeginAngle + i * tAngle) * radius, center2d.Y - sin(fBeginAngle + i * tAngle) * radius);
                core::vector2di end(center2d.X + cos(fBeginAngle + (i + 1) * tAngle) * radius, center2d.Y - sin(fBeginAngle + (i + 1) * tAngle) * radius);
                glBegin(GL_LINES);
                glColor4ub(color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha());
                glVertex2f(GLfloat(start.X), GLfloat(start.Y));
                glVertex2f(GLfloat(end.X), GLfloat(end.Y));
		...
		void COpenGLDriver::draw3DVerticalLine( const core::vector3d<f32> mid3d, /* 垂足点 */ const core::vector3d<f32> start3d, /* 三角形顶点 */ const core::vector3d<f32> end3d, /* 三角形顶点 */ f32 radius, /* 垂足线段长度 */ SColor color /*= SColor(255,255,255,255)*/ )
		{
			disableTextures();
			setRenderStates3DMode();

			core::vector3df tmpCenter(mid3d.X,mid3d.Y,mid3d.Z);
			core::vector3df samt(start3d.X,start3d.Y,start3d.Z);
			core::vector3df endt(end3d.X,end3d.Y,end3d.Z);
			...
			// 由法向量及垂足距离值，获得垂足点与两直角边的交点值
			core::vector3df verticalStart;// 水平垂线的交点(起始点)
			core::vector3df verticalEnd; // 垂直垂线的交点（终止点）
			core::vector3df verticalMid; // 垂线交点
			verticalStart = tmpCenter + samt * radius;
			verticalEnd = tmpCenter + endt * radius;
			verticalMid = tmpCenter + samt * radius +  endt * radius;
			// 获得垂线交点
			//core::vector3df verticalMid(verticalStart.X,verticalStart.Y,verticalEnd.Z);
			...
			glBegin(GL_LINES);
			glColor4ub(r,g,b,a);
			glVertex3f(verticalStart.X, verticalStart.Y, verticalStart.Z);
			glVertex3f(verticalMid.X, verticalMid.Y, verticalMid.Z);
			glEnd();

			glBegin(GL_LINES);
			glColor4ub(r,g,b,a);
			glVertex3f(verticalMid.X, verticalMid.Y, verticalMid.Z);
			glVertex3f(verticalEnd.X,verticalEnd.Y,verticalEnd.Z);
			glEnd();
		}
