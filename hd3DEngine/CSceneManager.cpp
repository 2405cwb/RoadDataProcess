...
: ISceneNode(0, 0), Driver(driver), FileSystem(fs), GUIEnvironment(gui),
	...
	MeshCache(cache), CurrentRendertime(ESNRP_NONE), LightManager(0),m_bIsAnimating(false),
...
gui::IGUIFont* CSceneManager::GetBuiltInFont()
{
	//return GUIEnvironment->getFont("E:\\myFont.bmp");
	return  GUIEnvironment->getBuiltInFont();
	/*GUIEnvironment->getSkin()->setSize()*/
}

gui::IGUIFont* CSceneManager::GetBuiltDefaultFont()
{
	return GUIEnvironment->getBuiltDefaultFont();
}

gui::IGUIFont* CSceneManager::GetChineseFont()
{
	return  GUIEnvironment->getBuiltChineseFont();
}

gui::IGUIFont* CSceneManager::GetAdaptiveFont(const char* srcStr, bool& bHaveWideChar)
{
	gui::IGUIFont* pTmpFont = GUIEnvironment->getBuiltDefaultFont();
	bHaveWideChar = pTmpFont->IsExistWideChar(srcStr);
	if (bHaveWideChar)
	{
		return  GUIEnvironment->getBuiltChineseFont();
	}
...
//! Adds a camera scene node which is able to be controlled with the mouse similar
//! to in the Software FaroScene.
//! The returned pointer must not be dropped.
//! added by yaoli on 2012/05/21
ICameraSceneNode* CSceneManager::addCameraSceneNodeCus(scene::ESCENE_NODE_CUSANIMATOR_STYLE style,
	ISceneNode* parent,
	f32 rotateSpeed, f32 zoomSpeed, f32 translationSpeed, s32 id,
	bool makeActive)
{
	...
	if (node)
	{
		ISceneNodeAnimator* anm = new CSceneNodeAnimatorCameraCus(CursorControl,
			rotateSpeed, zoomSpeed, translationSpeed,style);
...
//Added by yaoli
ISceneNodeAnimatorCameraCus* CSceneManager::GetCameraSceneNodeAnimator(ESCENE_NODE_ANIMATOR_TYPE type)
{
	scene::ICameraSceneNode* cam = getActiveCamera();

	core::list<scene::ISceneNodeAnimator*> Animators = cam->getAnimators();
	scene::ISceneNodeAnimatorList::Iterator ait = Animators.begin();
	while (ait != Animators.end())
	{
		scene::ISceneNodeAnimator* anim = *ait;
		++ait;
		if (type == anim->getType())
		{
			scene::ISceneNodeAnimatorCameraCus* cus = static_cast<scene::ISceneNodeAnimatorCameraCus*>(anim);
			return cus;
		}
	}
...
bool CSceneManager::IsAnimating()
{
	return m_bIsAnimating;
}

void CSceneManager::SetAnimateState(bool bAnimating)
{
	...
	*/
	...
	// render default objects
	{
		...
		else
		{
            ...
            {
				// 刷新DOM
				if (SolidNodeList[i].Node->getType() == ESNT_HD_DOM)
				{
					/*if ((m_refreshType & HDView_DOM) == HDView_DOM)				*/	
					{
			...
			// 蔡红云 2013/10/29 为解决场景中增加一些二维节点，如测量线、
			// 标签时，然后显示3DBox节点时，盒子不透明的问题，
			// 首先先渲染点云
			for (i=0; i<SolidNodeList.size(); ++i)
			{   
				// 如果节点类型为点云则首先渲染 
                if (SolidNodeList[i].Node->getType() == ESNT_HD_SEADATA_POINT
                    || SolidNodeList[i].Node->getType() == ESNT_SCAN_POINT
                    || SolidNodeList[i].Node->getType() == ESNT_CLASSIFY_PTD
                    || SolidNodeList[i].Node->getType() == ESNT_TRIANGLE
					/*|| SolidNodeList[i].Node->getType() ==  ESNT_HD_SCREENSHOT*/)
				{
					/*if ((m_refreshType & HDView_PointCloud) == HDView_PointCloud)			*/		
					{
						SolidNodeList[i].Node->render();
					}
				}
			...
			for (i=0; i<SolidNodeList.size(); ++i)
			{
				...
				if (SolidNodeList[i].Node->getType() ==  ESNT_HD_SEADATA_POINT
                    ...
                    || SolidNodeList[i].Node->getType() == ESNT_TRIANGLE
					/*|| SolidNodeList[i].Node->getType() ==  ESNT_HD_SCREENSHOT
					|| SolidNodeList[i].Node->getType() ==  ESNT_HD_DOM*/)
				{
					continue;
				}
				SolidNodeList[i].Node->render();
				//// 刷新全景
				//if (SolidNodeList[i].Node->getType() ==  ESNT_PANO)
				//{
				//	if ((m_refreshType & HDView_PANO) == HDView_PANO)					
				//	{
				//		SolidNodeList[i].Node->render();
				//	}
				//}
				...
				//// 刷新矢量图
				//if (SolidNodeList[i].Node->getType() ==  ESNT_OGRL_SCENE_NODE ||
				//	SolidNodeList[i].Node->getType() ==  ESNT_CONTOUR_LINES)
				//{
				//	if ((m_refreshType & HDView_PANO) == HDView_PANO)					
				//	{
				//		SolidNodeList[i].Node->render();
				...
				// 刷新DEM
	/*			if (SolidNodeList[i].Node->getType() ==  ESNT_HD_DEM ||
					SolidNodeList[i].Node->getType() ==  ESNT_HD_TIN ||
					SolidNodeList[i].Node->getType() ==  ESNT_HD_CUTFILL)
				{
					if ((m_refreshType & HDView_DEM) == HDView_DEM)					
					{
						SolidNodeList[i].Node->render();
					}
				}		*/					
				...
				// 刷新HDVIEW_Graphics
				//if ((m_refreshType & HDVIEW_Graphics) == HDVIEW_Graphics)					
				//{
				//	SolidNodeList[i].Node->render();
				//}
			}
...
//! 根据屏幕坐标获取视椎体
void CSceneManager::GetViewFrustum(const irr::core::recti& srcRect,SViewFrustum* pViewFrustum)
{
	ISceneCollisionManager* pSceneCol = getSceneCollisionManager();
	core::position2di tmpPos;
	tmpPos.set(srcRect.UpperLeftCorner.X,srcRect.UpperLeftCorner.Y);
	irr::core::line3df rayLT = pSceneCol->getRayFromScreenCoordinates(tmpPos);
	tmpPos.set(srcRect.UpperLeftCorner.X,srcRect.LowerRightCorner.Y);
	irr::core::line3df rayLB = pSceneCol->getRayFromScreenCoordinates(tmpPos);
	tmpPos.set(srcRect.LowerRightCorner.X,srcRect.LowerRightCorner.Y);
	irr::core::line3df rayRB = pSceneCol->getRayFromScreenCoordinates(tmpPos);
	tmpPos.set(srcRect.LowerRightCorner.X,srcRect.UpperLeftCorner.Y);
	irr::core::line3df rayRT = pSceneCol->getRayFromScreenCoordinates(tmpPos);

	core::vector3df nearLT,nearLB,nearRB,nearRT;//1 VF_NEAR_PLANE
	pViewFrustum->planes[1].getIntersectionWithLine(rayLT.start,rayLT.getVector(),nearLT);
	pViewFrustum->planes[1].getIntersectionWithLine(rayLB.start,rayLB.getVector(),nearLB);
	pViewFrustum->planes[1].getIntersectionWithLine(rayRB.start,rayRB.getVector(),nearRB);
	pViewFrustum->planes[1].getIntersectionWithLine(rayRT.start,rayRT.getVector(),nearRT);

	core::vector3df farLT,farLB,farRB,farRT;//0 VF_FAR_PLANE
	pViewFrustum->planes[0].getIntersectionWithLine(rayLT.start,rayLT.getVector(),farLT);
	pViewFrustum->planes[0].getIntersectionWithLine(rayLB.start,rayLB.getVector(),farLB);
	pViewFrustum->planes[0].getIntersectionWithLine(rayRB.start,rayRB.getVector(),farRB);
	pViewFrustum->planes[0].getIntersectionWithLine(rayRT.start,rayRT.getVector(),farRT);

	pViewFrustum->planes[2].setPlane(nearLB,nearLT,farLT);//VF_LEFT_PLANE
	pViewFrustum->planes[3].setPlane(farRT,nearRT,nearRB);//VF_RIGHT_PLANE
	pViewFrustum->planes[4].setPlane(nearRB,nearLB,farLB);//VF_BOTTOM_PLANE
	pViewFrustum->planes[5].setPlane(nearLT,nearRT,farRT);//VF_TOP_PLANE
	pViewFrustum->recalculateBoundingBox();
}
...
// 设置刷新渲染类型
void CSceneManager::SetRefreshType(ESCENE_REFRESH_TYPE type)
{
	m_refreshType = type;
}
ESCENE_REFRESH_TYPE CSceneManager::GetRefreshType( )
{
