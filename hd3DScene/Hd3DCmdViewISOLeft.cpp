...
namespace hd
{
	namespace scene
	{
		CHd3DCmdViewISOLeft::CHd3DCmdViewISOLeft(void)
			:CHdCommand("viewISOleft","左视图","viewISOleft","左视图",E_HCT_3D | E_HVT_3DVIEWMATCH)
		{
			m_id = COMMAND_3D_ISOLEFT;
		}
		...
		void CHd3DCmdViewISOLeft::OnClick()
		{
			if (!GetEnable())
				return;
			...
			if (hdView)
			{
				...
				core::vector3df newpos(viewBox.MaxEdge.X,viewBox.MaxEdge.Y,viewBox.MaxEdge.Z);
				core::vector3df newtar(viewBox.MinEdge.X,viewBox.MaxEdge.Y,viewBox.MinEdge.Z);
				...
				// chy 2014/4/16
				//CHd3DCamera* p3DCamera = dynamic_cast<CHd3DCamera*>(pCam);
				...
				if (( p3DCamera->GetCameraPosType() == E_CP_ISO_INVER_LEFT &&::GetKeyState(VK_CONTROL) < 0)||
					(p3DCamera->GetCameraPosType() == E_CP_ISO_LEFT && ::GetKeyState(VK_CONTROL) >= 0))
				{
					return;
				}
				...
				if (::GetKeyState(VK_CONTROL) < 0)
				{
					...
					p3DCamera->SetCameraPosType(E_CP_ISO_INVER_LEFT);
				...
				else
				{
					...
					p3DCamera->SetCameraPosType(E_CP_ISO_LEFT);
		...
		void CHd3DCmdViewISOLeft::OnCreate( CHdApp* app )
		{
			if(app == NULL)
				return;
			m_app = app;
		...
		bool CHd3DCmdViewISOLeft::GetEnable() 
		{
			m_enabled = false;
