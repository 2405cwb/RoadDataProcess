...
public:
    ...
    // 数据源是否支持用缩略图参与快速浏览。
    virtual bool usesThumbnailPreview() const { return true; }
    // 静止浏览时是否必须显示原图。普通文件图片返回 true；ImagePack
    // 等数据源可以继续由视图根据 LOD 决定是否加载完整图像。
    virtual bool requiresFullResolutionWhenIdle() const { return false; }
};
...
public:
    ...
    bool requiresFullResolutionWhenIdle() const override { return true; }

private:
    QVector<SequenceFrameDescriptor> m_frames; // 按采集/输入顺序保存帧元数据。
};

// 当前 ImagePack 图片序列数据源。
// 与 FileSequenceFrameSource 一样只负责“取帧 + 解码”，从 ISequenceFrameSource 往后显示链路完全一致。
class PackSequenceFrameSource : public ISequenceFrameSource
{
public:
    static QSharedPointer<PackSequenceFrameSource> create(const QString& packPath,
        bool verifyOnOpen, QString* errorMessage = nullptr);
...
private:
    ...
    ImagePackReader* readerForCurrentThread();

    QString m_indexPath;                             // 当前正式 img_<tag>.jph 的规范化绝对路径。
    bool m_verifyOnOpen = false;                     // 打开时是否执行 Pack 完整性校验。
    QVector<SequenceFrameDescriptor> m_frames;       // Pack 索引转换后的帧元数据。
    QThreadStorage<ImagePackReader*> m_threadReaders; // 解码工作线程各自持有独立 Reader，避免跨线程共享句柄。
};
