...
namespace hd
{
  ...
  computeRoots (const Matrix& m, Roots& roots)
  {
    ...
    else
    {
      ...
      Scalar theta = std::atan2 (sqrt (-q), half_b) * s_inv3;
  ...
  eigen33 (const Matrix& mat, typename Matrix::Scalar& eigenvalue, Vector& eigenvector)
  {
    ...
    Matrix::Scalar len1 = vec1.squaredNorm ();
    Matrix::Scalar len2 = vec2.squaredNorm ();
    Matrix::Scalar len3 = vec3.squaredNorm ();

    if (len1 >= len2 && len1 >= len3)
      eigenvector = vec1 / sqrt (len1);
    else if (len2 >= len1 && len2 >= len3)
      eigenvector = vec2 / sqrt (len2);
    else
      eigenvector = vec3 / sqrt (len3);
  }
