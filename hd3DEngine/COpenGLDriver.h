...
namespace video
{
	...
	public:
		...
		//! 绘制三维空间圆形-汪开理
		void drawCircle(const core::vector3d<f32> center3d, f32 radius, const core::vector3d<f32> normal3d, u32 count, SColor color = SColor(255,255,255,255));
        ...
        //! 绘制二维虚线段(draw2dDashedLine接口绘制虚线时无法再AMD显卡下显示)
        virtual void draw2dDashedLineAMD(const core::position2d<s32>& start, 
            const core::position2d<s32>& end, SColor color=SColor(255,255,255,255));

		// 绘制三维圆弧(圆弧中心点，起始圆弧射线点，终止圆弧的射线点（射线点不需要在圆上）)-2014.04.11.zhubo
		virtual void draw3DArc(const core::vector3d<f32> center3d,  // 圆弧所在圆的圆心
			const core::vector3d<f32> arcStart ,                    // 圆心与圆弧起始点构成的射线上一点（三角形顶点）
			const core::vector3d<f32> arcEnd ,                      // 圆心与圆弧终止点构成的射线上一点（三角形除圆心另一顶点）
			f32 radius,                                             // 设置圆弧半径
			f32 fAngle,                                             // 圆弧角度单位为度
