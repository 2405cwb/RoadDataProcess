...
public:
	...
	// 经纬度转换空间直角坐标系;
	int TransLators_BLToXYZ(double degree_latitude, double degree_longtitude,double phereHeight,double* out_X,double* out_Y,double* out_Z);

	// 空间直角坐标系转换经纬度;
	int TransLators_XYZToBL(double srcX,double srcY,double srcZ,double* out_latitude, double* out_longtitude,double* out_phereHeight);

	// 经纬度转换投影坐标,bOnly = true不考虑七参数等信息;
	int TranslatorBLToNeH(double in_l,double in_b,double in_h,double* out_e,double* out_n,double* out_h,bool bOnly = true);
	...
	int     TranslatorXYZByParam(double in_x, double in_y, double in_z, double *out_x, double *out_y, double *out_z);

	// 投影坐标经反向转换得到空间直角坐标;
	int     TranslatorXYZByParamInverse(double in_x, double in_y, double in_z, double *out_x, double *out_y, double *out_z);
