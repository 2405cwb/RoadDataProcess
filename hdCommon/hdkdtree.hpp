...
#ifndef  HDKDTREE_HPP_
#define  HDKDTREE_HPP_
...
namespace hdkdtree
{
	...
	/** Manhattan distance functor (generic version, optimized for high-dimensionality data sets).
	  *  Corresponding distance traits: hdkdtree::metric_L1
	...
	struct L1_Adaptor
	{
		...
		inline DistanceType operator()(const T* a, const size_t b_idx, size_t size, DistanceType worst_dist = -1) const
		{
			...
			while (a < lastgroup) {
				const DistanceType diff0 = hdkdtree::abs(a[0] - data_source.kdtree_get_pt(b_idx,d++));
				const DistanceType diff1 = hdkdtree::abs(a[1] - data_source.kdtree_get_pt(b_idx,d++));
				const DistanceType diff2 = hdkdtree::abs(a[2] - data_source.kdtree_get_pt(b_idx,d++));
				const DistanceType diff3 = hdkdtree::abs(a[3] - data_source.kdtree_get_pt(b_idx,d++));
			...
			while (a < last) {
				result += hdkdtree::abs( *a++ - data_source.kdtree_get_pt(b_idx,d++) );
			}
			return result;
		}
	...
	/** Squared Euclidean distance functor (generic version, optimized for high-dimensionality data sets).
	  *  Corresponding distance traits: hdkdtree::metric_L2
	  * \tparam T Type of the elements (e.g. double, float, uint8_t)
	  * \tparam DistanceType Type of distance variables (must be signed) (e.g. float, double, int64_t)
	  */
	template<class T, class DataSource, typename _DistanceType = T>
	...
	/** Squared Euclidean distance functor (suitable for low-dimensionality datasets, like 2D or 3D point clouds)
	  *  Corresponding distance traits: hdkdtree::metric_L2_Simple
	  * \tparam T Type of the elements (e.g. double, float, uint8_t)
	  * \tparam DistanceType Type of distance variables (must be signed) (e.g. float, double, int64_t)
	  */
	template<class T, class DataSource, typename _DistanceType = T>
	...
	public:
		...
		{
			hdkdtree::KNNResultSet<DistanceType,IndexType> resultSet(num_closest);
			resultSet.init(out_indices, out_distances_sq);
			this->findNeighbors(resultSet, query_point, hdkdtree::SearchParams());
		}
	...
	/** A simple KD-tree adaptor for working with data directly stored in an Eigen Matrix, without duplicating the data storage.
	  ...
	  *  \tparam Distance The distance metric to use: hdkdtree::metric_L1, hdkdtree::metric_L2, hdkdtree::metric_L2_Simple, etc.
	  *  \tparam IndexType The type for indices in the KD-tree index (typically, size_t of int)
	  */
	template <class MatrixType, int DIM = -1, class Distance = hdkdtree::metric_L2, typename IndexType = size_t>
	struct KDTreeEigenMatrixAdaptor
	{
		...
		KDTreeEigenMatrixAdaptor(const int dimensionality, const MatrixType &mat, const int leaf_max_size = 10) : m_data_matrix(mat)
		{
			...
			index = new index_t( dims, *this /* adaptor */, hdkdtree::KDTreeSingleIndexAdaptorParams(leaf_max_size, dims ) );
			index->buildIndex();
		}
		...
		{
			hdkdtree::KNNResultSet<typename MatrixType::Scalar,IndexType> resultSet(num_closest);
			resultSet.init(out_indices, out_distances_sq);
			index->findNeighbors(resultSet, query_point, hdkdtree::SearchParams());
		}
