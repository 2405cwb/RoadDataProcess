...
BOOL LASreaderASC::open(const char* file_name)
{
  ...
  while (!complete)
  {
    ...
    else if ((ncols != 0) && (nrows != 0) && (((xllcorner != F64_MAX) && (yllcorner != F64_MAX)) || ((xllcenter != F64_MAX) && (xllcenter != F64_MAX))) && (cellsize > 0))
    {
      ...
      {
        F32 e0, e1, e2;
        if ( sscanf_s(line, "%f %f %f", &e0, &e1, &e2) == 2)
        {
          complete = TRUE;
        }
      }
      ...
      {
        F32 e0, e1, e2, e3;
        if ( sscanf_s(line, "%f %f %f %f", &e0, &e1, &e2, &e3) == 3)
        {
          complete = TRUE;
        }
      }
      ...
      {
        F32 e0, e1, e2, e3, e4;
        if ( sscanf_s(line, "%f %f %f %f %f", &e0, &e1, &e2, &e3, &e4) == 4)
        {
          complete = TRUE;
        }
      }
      ...
      {
        F32 e0, e1, e2, e3, e4;
        if ( sscanf_s(line, "%f %f %f %f %f", &e0, &e1, &e2, &e3, &e4) == 5)
        {
          complete = TRUE;
        }
      }
  ...
  for (row = 0; row < nrows; row++)
  {
    ...
    for (col = 0; col < ncols; col++)
    {
      ...
      // get elevation value
      sscanf_s(&(line[line_curr]), "%f", &elevation);
      // skip parsed number
      while ((line[line_curr] != '\0') && (line[line_curr] != ' ')) line_curr++;
