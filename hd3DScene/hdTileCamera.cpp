...
namespace hd
{
	namespace fm
	{
		CHdTileCamera::CHdTileCamera(void)
			:CHdCamera("CHdTileCamera", "切片浏览工具", "CHdTileCamera", "切片浏览工具", E_HCT_Tile)
		{
			m_app = NULL;
			m_id = COMMAND_TILE_CAMERA;
			m_meshSN = NULL;
			m_type = E_HCT_Tile ;
		...
		CHdTileCamera::CHdTileCamera(const char* caption,const char* msg,const char* name,const char* tooltip, int type)
		...
		void CHdTileCamera::OnMouseDown( int Button, int Shift, int X, int Y )
		{
			if (!GetEnable())
				return;
		...
		void CHdTileCamera::OnMouseUp( int Button, int Shift, int X, int Y )
		{
			if (!GetEnable())
				return;
			...
			if (Button == 1)
			{
				...
				{
					pView->SendMsgToWindowsView(WM_USER_RELOAD_TITLEPANO, NULL, NULL);
				}
		...
		void CHdTileCamera::OnMouseMove( int Button, int Shift, int X, int Y )
		{
			if (!GetEnable())
				return;
		...
		void CHdTileCamera::OnCreate( CHdApp* app )
		{
			if(app == NULL)
				return;
			m_app = app;
		...
		bool CHdTileCamera::OnContextMenu( int X, int Y )
		{
			return true;
		}

		void CHdTileCamera::OnDblClick( int Button, int Shift, int X, int Y )
		{
		...
		void CHdTileCamera::OnMouseWheel( UINT nFlags, short zDelta, int X, int Y )
		{
			if (!GetEnable())
				return;
			...
			if (pView != NULL)
			{
				...
				if (fCurrentZoom != 0)
				{
					...
					else if (curFov > core::PI*2/3.0f)///和条件语句一致，修正鼠标滚轮缩放到一定倍率后滚轮失效的bug，by liuzhaoliang
					{
					...
					// 若包含全景切片sn，刷新时更新数据纹理贴图
					if (pView->IsIncludeSceneNode(ESNT_PANO))
					{
						// 先更新再刷新
		...
		void CHdTileCamera::OnKeyDown( int keyCode, int Shift )
		{
		...
		void CHdTileCamera::OnKeyUp( int keyCode, int Shift )
		{

		}

		//! 得到相机角度
		void CHdTileCamera::GetViewAngle(core::vector3df& angle)
		{
		...
		void CHdTileCamera::MoveCamera(core::vector3df newPos)
		{
		...
		void CHdTileCamera::MoveCamera( core::vector2df newPos )
		{
			CHdQuickView* pView = dynamic_cast<CHdQuickView*>(m_app->GetActiveView());
			if (pView == NULL)
			{
		...
		void CHdTileCamera::GetVectorAngle(core::vector3df vect,float& fViewX,float& fViewY)
		{
			vect.normalize();
		...
		void CHdTileCamera::GetCameraState(float& fViewX, float& fViewY, float& xFov, float& yFov)
		{
			ISceneView* pView = dynamic_cast<ISceneView*>(m_app->GetActiveView());
			if (pView == NULL)
			{
		...
		void CHdTileCamera::GetAngleBySC(int nScX,int nScY,float& fHoriAngle,float& fVertAngle)
		{
			ISceneView* pView = dynamic_cast<ISceneView*>(m_app->GetActiveView());
			if (pView == NULL)
			{
		...
		void CHdTileCamera::FallBack3DCameraStatus()
		{
			//在快速视图下，双击鼠标，相机退出全景球
		...
		void CHdTileCamera::CameraAdjust( int type )
		{
			if (!GetEnable())
				return;
		...
		void CHdTileCamera::InitViewAngle()
		{
			if (!GetEnable())
				return;
			ISceneView* pView = dynamic_cast<ISceneView*>(m_app->GetActiveView());
		...
		void CHdTileCamera::CameraAdjustByAngle(double fAngleH,double fAngleV)
		{
			if (!GetEnable())
				return;
			ISceneView* pView = dynamic_cast<ISceneView*>(m_app->GetActiveView());
